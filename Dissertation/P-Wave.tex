...
\beq
\SphericalHarmonicY{1}{0}{\theta_\rho}{\varphi_\rho} = \sqrt{\frac{3}{4\pi}} \cos \theta_\rho
\eeq

\beq
j_1(\kappa \rho) = \frac{\sin(\kappa\rho)}{(\kappa\rho)^2} - \frac{\cos(\kappa\rho)}{\kappa\rho}
\eeq

\beq
n_1(\kappa \rho) = -\frac{\cos(\kappa\rho)}{(\kappa\rho)^2} - \frac{\sin(\kappa\rho)}{\kappa\rho}
\eeq
...
The short-range -- long-range terms can be directly computed by changing the short-range terms, but the integration routines (section \ref{sec:ShortInt}) for the short-range -- short-range terms cannot handle the $\rho$ factor of $\bar{\phi}_{\rho 1}$ directly. The following relations must be used:  \todoi{Ref?}
...
%This work $(\omega \rightarrow \infty)$ $\delta_1^-$	& $-0.172^{-2}$ & $-0.165^{-1}$ & $-0.540^{-1}$ & $-0.114$ & $-0.182$ & $-0.246$ & $-0.281$ \\
...
CC 14Ps14H+H$^-$ \cite{Walters2004} $\delta_1^+$		& 0.221^{-1} & 0.183 & 0.580 & 0.956 & 1.106 & 1.134 & 1.133 \\
CC 14Ps14H \cite{Blackwood2002} $\delta_1^+$			& 0.213^{-1} & 0.175 & 0.545 & 0.908 & 1.068 & 1.103 & 1.099 \\
3-state CC \cite{Sinha1997} $\delta_1^+$				& 8.14^{-3} & 6.27^{-2} & 1.90^{-1} & 3.58^{-1} & 4.89^{-1} & 5.51^{-1} & 5.56^{-1} \\
SE \cite{Ray1997} $\delta_1^+$ 							& 0.798^{-2} & 0.614^{-1} & 0.186 & 0.349 & 0.477 & 0.536 & 0.538 \\
5-state CC \cite{Adhikari1999} $\delta_1^+$				& 0.477^{-2} & 0.370^{-1} & 0.116 & 0.239 & 0.372 & 0.478 & 0.541 \\
...
\arrayrulecolor[RGB]{220,220,220}\midrule\arrayrulecolor{black}
CC 14Ps14H \cite{Blackwood2002} $\delta_1^-$			& -0.953^{-3} & -0.122^{-1} & -0.456^{-1} & -0.104 & -0.178 & -0.247 & -0.295 \\
3-state CC \cite{Sinha1997} $\delta_1^-$				& -4.43^{-3} & -3.08^{-2} & -8.51^{-2} & -1.59^{-1} & -2.36^{-1} & -3.02^{-1} & -3.32^{-1} \\
SE \cite{Ray1997} $\delta_1^-$							& -0.503^{-2} & -0.352^{-1} & -0.980^{-1} & -0.186 & -0.287 & -0.390 & -0.488 \\
5-state CC \cite{Adhikari1999} $\delta_1^-$				& -0.233^{-2} & -0.167^{-1} & -0.476^{-1} & -0.918^{-1} & -0.142 & -0.190 & -0.228 \\
...
\begin{tabular}{c c c}
\toprule
$\kappa$ & $1^{st}$ Formalism $\delta^+ (\omega = 6)$ & $2^{nd}$ Formalism $\delta^+ (\omega = 6)$ \\
\midrule
0.1 & $2.255^{-2}$ & $2.255^{-2}$ \\
0.2 & $1.909^{-1}$ & $1.908^{-1}$ \\
0.3 & $6.082^{-1}$ & $6.080^{-1}$ \\
...
\bottomrule
\end{tabular}
\caption{Comparison of $^1$P phase shifts for the first and second formalisms}
\label{tab:PWaveFormalPhase}
\end{table}

The phase shifts for $^1$P were not improved by using the second formalism and were even slightly lower in some cases. The energy eigenvalues in \cref{tab:SimplexPWaveSingOpt,tab:SimplexPWaveTripOpt} were also lower for the first formalism, so we did not pursue using the second formalism any further. The second formalism was not tried for the D-wave.
...
resonances before the inelastic threshold. Di Rienzi and Drachman found that 
the first $^1$P resonance is associated with the $3p$ state, not the $2p$ 
state as might be expected \cite{DiRienzi2002b}. These resonances are shifted 
to higher energies than the S-wave, and we use the numerical fittings described
in \cref{sec:ResonanceFit} to determine the resonance parameters given in
\cref{tab:PWaveResonancesOther}.
