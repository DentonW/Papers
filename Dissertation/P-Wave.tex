...
matrix elements with the barred versions of $S_1$ and $C_1$ for the Kohn, then
...
\left(\bar{S}_1,\mathcal{L}\bar{S}_1\right) = \pm \frac{\uppi}{2} \int \frac{1}{\rho\rho^\prime} & \left[S_{22} S_{23} \left(\frac{2}{r_1} - \frac{2}{r_2} - \frac{2}{r_{13}} + \frac{2}{r_{23}} \right) \right.  \nonumber \\
...
\left(\bar{C}_1,\mathcal{L}\bar{S}_1\right) = \pm \frac{\uppi}{2} \int \frac{1}{\rho\rho^\prime} & \left[C_{23} S_{22} \left(\frac{2}{r_1} - \frac{2}{r_2} - \frac{2}{r_{13}} + \frac{2}{r_{23}} \right) \right.  \nonumber \\
& \left. \times \left(r_1^2 + r_1 r_2 \cos\theta_{12} + r_1 r_3 \cos\theta_{13} + r_2 r_3 \cos\theta_{23} \right) \right] d\tau_{int}
\end{align}
...
directly. The following relations must be used:
\begin{subequations}
\label{eq:P2rhoY10}
\begin{align}
...
\Cref{tab:PWaveComparisons} compares the current complex Kohn results for the 
...
\% Diff entries give an estimate of the error for our results. The singlet 
...
complex Kohn and the CC phase shifts of the Belfast group appear to give the 
most accurate sets of phase shifts, and the current complex Kohn can 
potentially be viewed as benchmark results.
