...
obtain the phase shifts directly from 
...
%\end{figure}
%
%\Cref{fig:ScatteringDiagram} shows a general scattering diagram.
If we have azimuthal symmetry, as we assume for Ps-H scattering,
the results are independent of $\varphi$.
...
The integrated cross section %(also known simply as the cross section)
is related to the differential cross section by integrating over $d\Omega$:
\begin{equation}
...
expressed as \citep[p.584]{Bransden2003}
\begin{equation}
\label{eq:TotalCross}
\sigma_{el}^\pm = \sum_{\ell=0}^\infty \sigma_{el,\ell}^\pm = \frac{4}{\kappa^2} \sum_{\ell=0}^\infty (2\ell+1) \sin^2 \delta_\ell^\pm.
\end{equation}
We consider that the H(1s) is unpolarized and the final spin states are not
determined, giving spin-weighted cross sections where the
singlet contributes $\frac{1}{4}$, and the triplet contributes $\frac{3}{4}$,
i.e. \cite{Blackwood2002,Ward1987,Joachain1979,Ray1996}
\beq
...
wave cross sections for the singlet and triplet, respectively. The ``Summed''
...
higher partial waves contribute nearly negligible amounts. The 
summed cross section follows
...
from the first four partial waves. The ``Summed'' has peaks from each of the
first three partial waves, giving it a more complicated structure than the
summed from the triplet in \cref{fig:triplet-cross-sections}. As mentioned
...
higher partial wave resonances have a larger contribution to the summed
...
by the S-wave, as we would expect. At zero energy, the S-wave is the
only partial wave that has a non-zero cross section (for both $^1$S and $^3$S).
Bransden and Joachain \cite[p.589]{Bransden2003} point out that for $\ell \geq 1$,
...
Also interesting is the minimum in the summed singlet cross section at 
...
\begin{figure}
	...
	\caption[Singlet elastic partial wave cross sections and the sum]{$S$-matrix complex Kohn singlet elastic partial wave cross sections and the sum through $\ell = 5$. An interactive version of this
figure is available online \cite{Plotly} at
\url{https://plot.ly/~Denton/138/triplet-partial-cross-sections-ps-h-scattering/}.}
...
\begin{figure}
	...
	\caption[Triplet elastic partial wave cross sections and the sum]{$S$-matrix complex Kohn triplet elastic partial wave cross sections and the sum through $\ell = 5$. An interactive version of this
figure is available online \cite{Plotly} at
\url{https://plot.ly/~Denton/138/triplet-partial-cross-sections-ps-h-scattering/}.}
...
consider what the percentage contribution to the summed cross section for
...
the contribution to the elastic integrated cross section for
...
\begin{figure}
	...
	\caption[Percentage contribution to integrated elastic cross section for selected $\kappa$]{Percentage contribution to integrated elastic cross section from each partial wave with selected $\kappa$ for the singlet (a) and triplet (b).}
	\label{fig:percentage-cross-sections}
\end{figure}

\Cref{fig:percentage-cross-sections-full} also shows the percent contributions to 
the integrated elastic cross sections from each partial wave but shows
...
\begin{figure}
	...
	\caption[Percentage contribution to integrated elastic cross section]{Percentage contribution to integrated elastic cross section from each partial wave for the singlet (a) and triplet (b).}
	\label{fig:percentage-cross-sections-full}
\end{figure}

For a more quantitative approach, we calculate the percent contributions of the
different partial waves to the spin-weighted integrated elastic cross section as presented
...
partial wave's contribution to the spin-weighted integrated elastic cross section, but we
...
waves through the H-wave in the final results, but these would be relatively
...
%\bottomrule
%\end{tabular}
%\caption[Cross section contribution from each partial wave]{Percent contribution to the elastic integrated cross section from each partial wave for both the maximum and average for the entire energy range}
...
\toprule
& Max. & Avg. & Max. & Max. & Avg. & Max. & Max. & Avg. \\
$\ell$ & \% & \% & $^1\ell$ \% & $E_{\bm \kappa}^-$ (eV) & $\%^+$ & $\%^-$ & $E_{\bm \kappa}^+$ (eV) & $\%^-$ \\
\midrule
...
\bottomrule
\end{tabular}
\caption[Cross section contribution from each partial wave]{Percent contribution to the elastic integrated cross section from each partial wave for both the maximum and average for the entire energy range. Superscripts give powers of 10.}
\label{tab:PercentToCross}
\end{table}

Finally, combining the singlet and triplet integrated elastic cross 
...
and triplet cross sections for comparison with the combined integrated elastic cross section.
...
%energy, where it overestimates the cross section greatly.
There is
...
\begin{figure}
	...
	\caption[Elastic integrated cross sections]{Elastic integrated cross sections. CC results are from Ref.~\cite{Walters2004}, and SE results are from Ref. \cite{Hara1975}. An interactive version of this figure is available online \cite{Plotly}
at \url{https://plot.ly/~Denton/150/integrated-cross-sections-ps-h-scattering/}.}
...
with the complex Kohn results. However, there are some features that are worth
noticing. For the $^1$P cross section in \cref{fig:spd-singlet-cross-sections}, the CC
9Ps9H + H$^-$ maximum is lower than the complex Kohn maximum. The CC cross section
...
complex Kohn results, making them less negative and the corresponding cross sections
...
As a simple test, we tried replacing the complex Kohn $\kappa = 0.1 - 0.7$ $^3$D
...
\begin{figure}
	...
	\caption[Comparisons of $^1$S, $^1$P, and $^1$D cross sections.]{Comparisons of $^1$S, $^1$P, and $^1$D elastic partial wave cross sections. CC 9Ps9H + H$^-$ results are from Ref.~\cite{Walters2004}, and CC 22Ps1H + H$^-$ results are from Ref.~\cite{Blackwood2002b}.}
...
\begin{figure}
	...
	\caption[Comparisons of $^3$S, $^3$P, and $^3$D cross sections.]{Comparisons of $^3$S, $^3$P, and $^3$D elastic partial wave cross sections. CC results for $^3$S and $^3$P are from Ref. \cite{Walters2004}, and the CC results for $^3$D are from Ref. \cite{Blackwood2002}.}
...
%to see if the discrepancy is as large.
In \cref{fig:fgh-born-cross-sections}, the BO approximation elastic partial
wave cross sections do not match up with either the singlet
...
%without improving the accuracy of our long-range code.
Similar to what we
...
\begin{figure}
	...
	\caption[Comparisons of F-, G-, and H-wave cross sections]{Comparisons of F-, G-, and H-wave elastic partial wave cross sections between complex Kohn and BO approximation}
...
features are visible instead of being obscured by the higher value of the differential
cross section in the front of the graph.
...
forward direction $(\theta = 0)$. The triplet differential cross section in the
forward direction is less than 7 $a_0^2/\textrm{sr}$ for all energies considered.
...
at very low energy, as we would expect. For nearly zero energy at
...
\begin{figure}
	...
	\caption{Differential cross section for selected $\theta$}
...
isotropic, with a slight bias toward backward scattering. As $\kappa$ is %the momentum is 
...
backward direction, and a minimum at approximately $100\degree$. As %the momentum
$\kappa$ is increased further, the differential cross section becomes very 
...
\begin{figure}
	...
	\caption{Differential cross section for selected $\kappa$}
...
\cref{sec:totalcross}. The convergence of the differential cross section
is more difficult to quantitatively evaluate than that of the
integrated cross section, since it has mixing between all of the included
partial waves. %Due to the
%double summation, there are $\ell_{max}^2$ terms in the differential cross
%section.
We calculate $\frac{d\sigma_{el}}{d\Omega}$ for two subsequent
...
We would expect that as $\ell$ increases, $\theta = 180\degree$ 
...
\begin{figure}
	...
	\caption[Percent difference of differential cross section at all angles]{Percent difference of $\frac{d\sigma_{el}}{d\Omega}$ for upper limit of summations in \cref{eq:DiffCross} as $\ell_{max} = 3$ versus $\ell_{max} = 4$ for all angles and energies}
...
\begin{figure}
	...
	\caption[Percent difference of differential cross section at all angles]{Percent difference of $\frac{d\sigma_{el}}{d\Omega}$ for upper limit of summations in \cref{eq:DiffCross} as $\ell_{max} = 4$ versus $\ell_{max} = 5$ for all angles and energies}
...
\begin{figure}
	...
	\caption[Percent difference of differential cross section at selected angles]{Percent difference of $\frac{d\sigma_{el}}{d\Omega}$ for upper limit of summations in \cref{eq:DiffCross} as $\ell_{max} = 3$ versus $\ell_{max} = 4$ for selected angles}
...
\begin{figure}
	...
	\caption[Percent difference of differential cross section at selected angles]{Percent difference of $\frac{d\sigma_{el}}{d\Omega}$ for upper limit of summations in \cref{eq:DiffCross} as $\ell_{max} = 4$ versus $\ell_{max} = 5$ for selected angles}
...
significant contribution, and the differential cross section looks to be
...
Unsurprisingly, for $\ell \geq 1$, the angles that are the most
...
\section{Momentum Transfer Cross Section and Comparisons}
\label{sec:OtherCross}

Measurements of the momentum transfer cross section, $\sigma_m$,
have been made for Ps scattering with atomic and molecular targets
\cite{Engbrecht2008,Nagashima1998,Saito2003,Skalsey1998}, and calculations of
Ps scattering by inert gases have been performed \cite{Blackwood2002c}.
Calculations of momentum transfer cross sections for other systems
have been made in Refs.~\cite{Wang2014, McEachran2014}.
...
using the complex Kohn phase shifts. The spin-weighting is from \cref{eq:SpinWeightCS}.
...
\begin{figure}
	...
	\caption[Momentum transfer cross sections]{$S$-matrix complex Kohn momentum transfer cross sections}
...
%\end{equation}
%
%\todoi{Ortho-para in \cite{Baltenko1983}, \cite{Fraser1961}, and \cite{Biswas2002}}
%
%\begin{figure}
	%\centering
...
\begin{figure}
	...
	\caption[Comparison of cross sections.]{Comparison of cross sections. CC data is from Ref.~\cite{Walters2004}.}
	\label{fig:cross-section-comparisons}
\end{figure}

\Cref{fig:cross-section-comparisons} shows the elastic integrated and momentum
transfer cross sections.
%As expected,
$\sigma_m \approx \sigma_{el}$ at very low energy. 
...
As Blackwood et al.\ \cite{Blackwood2002c} note for Ps-Ne scattering,
this is due to the differential cross section being essentially isotropic
at low energy (and exactly isotropic at zero energy), as seen in
...
Past zero energy, where $\sigma_m = \sigma_{el}$, we see that $\sigma_m > \sigma_{el}$
...
%Data from \cite{Biswas1998} pulled form plot using WebPlotDigitizer \cite{WebPlotDigitizer}}
%
...
section with respect to $\ell$. The differential cross section
