...
\end{equation}
\todoi{Page number for Bransden book}
...
\end{equation}
We consider the target H to be unpolarized, so
...
\todoi{More complete figure captions}
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{diff-cross-sections-combined-1}
	\caption{Combined spin-weighted differential cross section}
...
\todoi{Note from Blackwood paper \cite{Blackwood2002c} on isotropic at low E (in our paper?)}
...
\end{figure}
\todoi{OP in \cite{Biswas1998}. The problem is that their y-axis is logarithmic.}
...
As expected, $\sigma_m \approx \sigma_{el}$ at very low energy. Specifically, for
$E_{\bm \kappa} < 10^{-6}$ eV, $\sigma_m = \sigma_{el} = 32.45$ $\pi a_0^2$. As
Blackwood et al.\ \cite{Blackwood2002c} note, this is due to the differential
cross section being essentially isotropic at low energy (and exactly equal at
...
in the forward and backward directions. This effect can slightly be seen in
\cref{fig:diff-cross-sections-combined}, but it is most clearly shown in
\cref{fig:diff-cross-section-2D-theta}.


As noticed in the differential cross section, 
