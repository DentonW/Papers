...
If the ``mixed terms'' in \cref{sec:MixedTerms} are not included for the
...
As shown in Appendix \ref{chp:AngularInt}, the external angular integrations 
...
$\mathcal{L}$ on the long-range parts. Section \ref{sec:GeneralShort} covers 
...
spherical harmonics using the Laplacian formalism instead.
It is this approach that we take here. All equations 
used from their paper have been rederived in my notes \cite{Wiki}.
...
\noindent As worked out in my notes and shown in their paper, when the
Laplacians are expanded, this is given by the form
\begin{align}
\label{eq:GenHam2}
...
\end{align}
As noted on page~\pageref{BraNote} for the Kohn variational methods, these
