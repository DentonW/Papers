...
The $(\widetilde{C},\mathcal{L}\widetilde{S})$ and $(\widetilde{\phi}_i,\mathcal{L}\widetilde{S})$ matrix elements have the $\mathcal{L}$-operator acting on $S$. While not explicitly in the matrix given by \cref{eq:GeneralKohnMatrix}, $(\widetilde{S},\mathcal{L}\widetilde{S})$ is required in the final \cref{eq:GenDFDT2}. Also from the general $\widetilde{S}$ and $\widetilde{C}$, \cref{eq:GenSCMatrix} gives that the evaluation of $(\widetilde{S},\mathcal{L}\widetilde{S})$ is needed. We first look at $\mathcal{L}$ operating on $S$ and then determine the form of these matrix elements.
...
%\beq
%(\bar{S}_\ell,\mathcal{L}\bar{S}) = \frac{1}{2} \left((S \pm S^\prime),\mathcal{L}(S \pm S^\prime)\right) = \frac{1}{2} \left[(S,\mathcal{L}S) \pm (S,\mathcal{L}S^\prime) \pm (S^\prime,\mathcal{L}S) + (S^\prime,\mathcal{L} S^\prime) \right]
%\eeq
%
...
%
%%\beq
%%(\bar{S}_\ell,\mathcal{L}\bar{S}) = \pm \left(S^\prime,\mathcal{L}S\right) = \pm \left(S^\prime, \left[ \frac{2}{r_1} - \frac{2}{r_2} - \frac{2}{r_{13}} + \frac{2}{r_{23}} \right] S\right)
%%\label{eq:SbarLSbar}
%%\eeq
%
%%The same computation applies for $\bar{C}$:
%%\beq
%%(\bar{C}_\ell,\mathcal{L}\bar{S}) = \pm \left(C^\prime,\mathcal{L}S\right) = \pm \left(C^\prime, \left[ \frac{2}{r_1} - \frac{2}{r_2} - \frac{2}{r_{13}} + \frac{2}{r_{23}} \right] S\right)
%%\label{eq:CbarLSbar}
%%\eeq
%
%We can use (\ref{eq:SLCandCLS}) to find $(\bar{S}_\ell,\mathcal{L}\bar{C})$ from this:
%\beq
%\left(\bar{S}_\ell,\mathcal{L}\bar{C}\right) = \left(\bar{C}_\ell,\mathcal{L}\bar{S}\right) + 1
...
\begin{align}
\left(\bar{C}_\ell,\mathcal{L}\bar{C}\right) = \frac{1}{2}\left[2(C_\ell,\mathcal{L}C_\ell) \pm 2(C_\ell',\mathcal{L}C_\ell)\right] 
 = (C_\ell,\mathcal{L}C_\ell) \pm (C_\ell',\mathcal{L}C_\ell).
 \label{eq:CLC1}
\end{align}

Substitute (\ref{eq:CDef}) and (\ref{eq:LC}) in (\ref{eq:CLC1}) to get
\begin{align}
\left(\bar{C}_\ell,\mathcal{L}\bar{C}_\ell\right) = &(C_\ell,\mathcal{L}C_\ell) \pm (C_\ell',\mathcal{L}C_\ell) = \left((C_\ell \pm C_\ell'),\mathcal{L}C_\ell \right) \nonumber\\
 = &\left((C_\ell \pm C_\ell') \Phi_{Ps}(r_{12}) \Phi_H(r_3) \sqrt{\frac{2\kappa}{4\pi}} \left\{ \left(\frac{2}{r_1} - \frac{2}{r_2} - \frac{2}{r_{13}} + \frac{2}{r_{23}} \right) \frac{\cos(\kr)}{\kr} \right.\right.  \nonumber \\
...
\begin{align}
\left(\bar{C}_\ell,\mathcal{L}\bar{C}_\ell\right) = &\sqrt{\frac{2\kappa}{4\pi}} \frac{\kappa\mu}{2} \left((C_\ell \pm C_\ell^\prime) e^{-\mr}(1+\mr) \frac{\sin(\kr)}{\kr} \Phi_{Ps}(r_{12}) \Phi_H(r_3) \right)  \nonumber\\
 &+ \sqrt{\frac{2\kappa}{4\pi}} \frac{\mu^3}{4} \left((C_\ell \pm C_\ell') e^{-\mr} \frac{\cos(\kr)}{\kr} \Phi_{Ps}(r_{12}) \Phi_H(r_3) \right) \nonumber\\
 &+ \sqrt{\frac{2\kappa}{4\pi}} \left((C_\ell \pm C_\ell') \left(\frac{2}{r_1} - \frac{2}{r_3} - \frac{2}{r_{12}} + \frac{2}{r_{23}}\right) \frac{\cos(\kr)}{\kr} \right.  \nonumber \\
 &\times \left. \left[1 - e^{-\mr}\left(1 + \frac{\mu}{2}\rho \right)\right] \Phi_{Ps}(r_{12}) \Phi_H(r_3) \right) \nonumber\\
= &\frac{\kappa\mu}{2} \left((C_\ell \pm C_\ell^\prime) e^{-\mr}(1+\mr) S_\ell \right) \nonumber\\
 &+ \sqrt{\frac{2\kappa}{4\pi}} \frac{\mu^3}{4} \left((C_\ell \pm C_\ell^\prime) e^{-\mr} \frac{\cos(\kr)}{\kr} \Phi_{Ps}(r_{12}) \Phi_H(r_3) \right) \nonumber\\
 &+ \left((C_\ell \pm C_\ell^\prime) \left(\frac{2}{r_1} - \frac{2}{r_3} - \frac{2}{r_{12}} + \frac{2}{r_{23}}\right) C_\ell \right).
 \label{eq:CBarLCBar2}
\end{align}

Looking at just the last term:
\begin{align}
&\left((C_\ell \pm C_\ell') \left(\frac{2}{r_1} - \frac{2}{r_3} - \frac{2}{r_{12}} + \frac{2}{r_{23}}\right) C_\ell \right) \nonumber \\
 &\phantom{Space} = \left( \left(\frac{2}{r_1} - \frac{2}{r_3} - \frac{2}{r_{12}} + \frac{2}{r_{23}}\right) C_\ell^2 \right) \pm \left( \left(\frac{2}{r_1} - \frac{2}{r_3} - \frac{2}{r_{12}} + \frac{2}{r_{23}}\right) C_\ell' C_\ell \right)
\end{align}

\noindent The first set of parentheses has the same form as \cref{eq:SbarLSbar}. These terms in parentheses are antisymmetric with respect to the $1 \leftrightarrow 2$ permutation. Also, $C_\ell$ is symmetric with respect to this permutation. So the first set of parentheses is 0.  Thus,
\beq
\left((C_\ell \pm C_\ell') \left(\frac{2}{r_1} - \frac{2}{r_3} - \frac{2}{r_{12}} + \frac{2}{r_{23}}\right) C_\ell \right) = \pm \left(C_\ell' \left(\frac{2}{r_1} - \frac{2}{r_3} - \frac{2}{r_{12}} + \frac{2}{r_{23}}\right) C_\ell \right)
\eeq

\noindent We finally have that
\begin{align}
\left(\bar{C}_\ell,\mathcal{L}\bar{C}_\ell\right) = \: &\frac{\kappa\mu}{2} \left((C_\ell \pm C_\ell') e^{-\mr}(1+\mr) S_\ell \right) \nonumber\\
 &+ \sqrt{\frac{2\kappa}{4\pi}} \frac{\mu^3}{4} \left((C_\ell \pm C_\ell') e^{-\mr} \frac{\cos(\kr)}{\kr} \Phi_{Ps}(r_{12}) \Phi_H(r_3) \right) \nonumber\\
 &\pm \left(C_\ell' \left(\frac{2}{r_1} - \frac{2}{r_3} - \frac{2}{r_{12}} + \frac{2}{r_{23}}\right) C_\ell \right).
 \label{CBarLCBar}
\end{align}

\textbf{@TODO:} Put this elsewhere.
We chose to absorb both the $\frac{1}{\sqrt{2}}$ and $Y_{0,0}(\theta_\rho,\varphi_\rho) = \frac{1}{\sqrt{4\pi}}$ into the $c_i$ constants of the short-range terms in \cref{eq:PhiDef}.


The analysis for $(\bar{\phi}_i, \mathcal{L}\bar{C}_\ell)$ and $(\bar{C}_\ell, L\bar{\phi}_i)$ is similar to the analysis in \cref{sec:LSElements}.
\begin{subequations}
\begin{align}
(\bar{\phi}_i, \mathcal{L}\bar{C}_\ell) &= \frac{2}{\sqrt{2}} \left[(\phi_i,\mathcal{L}C_\ell) \pm (\phi_i',\mathcal{L}C_\ell)\right] \label{PhiBarLCBar2a} \\
 &= \frac{2}{\sqrt{2}} \left[(\phi_i,\mathcal{L}C_\ell) \pm (\phi_i,\mathcal{L}C_\ell')\right]  \label{PhiBarLCBar2b}
\end{align}
\end{subequations}

Using \cref{eq:LC,eq:LCP}) in the above gives our final result for $(\bar{\phi}_i, \mathcal{L}\bar{C})$.
\begin{subequations}
\begin{align}
(\bar{\phi}_i, \mathcal{L}\bar{C}_\ell) = \sqrt{\frac{\kappa}{\pi}} &\left( (\phi_i \pm \phi_i') \Phi_{Ps}(r_{12}) \Phi_H(r_3) \left\{ \left(\frac{2}{r_1} - \frac{2}{r_2} - \frac{2}{r_{13}} + \frac{2}{r_{23}}\right) \frac{\cos(\kr)}{\kr} \left[1 - e^{-\mr}\left(1 + \frac{\mu}{2}\rho\right)\right] \right.\right. \nonumber\\
