...
Our complex Kohn phase shifts at $\omega = 7$ from this table are exactly the same as Van Reeth and Humberston's results for $\omega = 6$ \cite{VanReeth2003}, with some exceptions in the last digit. This indicates that the prior Kohn variational method S-wave phase shifts were well converged. \Cref{fig:SWavePhase} has the more complete set of phase shifts plotted with respect to the incoming Ps energy, $E_{\bm \kappa}$. This compares the complex Kohn phase shifts to that of the $^1$S CC \cite{Walters2004} and the $^3$S CC \cite{Blackwood2002}, along with the $^{1,3}$S CVM \cite{Zhang2012}.

\begin{figure}[H]
	\centering
	\includegraphics[width=7in]{swave-phases}
	\caption{$^{1,3}$S complex Kohn phase shifts. The $^1S$ CC phase shifts
...
In the inset in \cref{fig:SWavePhase}. 
...
\midrule
Kohn $\omega = 6$ \cite{VanReeth2003} & $-0.427$ & $-0.820$ & $-1.161$ & $-1.446$ & $-1.677$ & $-1.857$ & $-1.964$ \\
\midrule
...
CC 14Ps14H+H$^-$ \cite{Blackwood2002} $\delta_0^-$ & $-0.206$ & $-0.414$ & $-0.624$ & $-0.838$ & $-1.037$ & $-1.213$ & $-1.367$ \\
\bottomrule
\end{tabular}
\caption{$^1$S Results from other groups. Values in the header are $\kappa$ in au.}
...
\todoi{Discussion of Peter's previous calculations and omitted terms. Elsewhere.}
