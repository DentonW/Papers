...
%\label{eq:LC}
%\end{align}
Combining \cref{eq:LCMathS,eq:LC} and the permuted versions gives
\begin{align}
...
Substitute \cref{eq:GenCDef,eq:LC} in \cref{eq:CLC1} and simplify to get
%\begin{align}
...
constants of the short-range terms in \cref{eq:PhiDefS}.
...
\noindent Using \cref{eq:LC,eq:LCBar} in the above gives our results for $(\bar{\phi}_i, \mathcal{L}\bar{C}_0)$.
\begin{subequations}
\begin{align}
...
\bottomrule
\end{tabular}
\caption{Comparison of $^{1,3}$S phase shifts between complex Kohn results and those from other groups. Values in the header are $\kappa$ in a.u.}
