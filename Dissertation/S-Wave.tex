...
\mathcal{L}_v = \mathcal{L}_t - (\psi_t,L \psi_t) = \mathcal{L}_t - \Big((\bar{\bar{S}} + \mathcal{L}_t \tilde{C} + \sum_i c_i \tilde{\phi}_i), L (\tilde{S} + \mathcal{L}_t \tilde{C} + \sum_j c_j \tilde{\phi}_j )\Big).
\eeq
...
\beq
0 = \frac{\partial \mathcal{L}_v}{\partial \mathcal{L}_t} = 1 - \Big[(\tilde{S},L\tilde{C}) + (\tilde{C},L\tilde{S}) + \frac{\partial}{\partial \mathcal{L}_t}(\mathcal{L}_t \tilde{C},L \mathcal{L}_t \tilde{C}) + (\tilde{C}, L \sum_i c_i \tilde{\phi}_i) + (\sum_i c_i \tilde{\phi}_i, L \tilde{C}) \Big].
\label{eq:PdLambda1}
\eeq

\noindent The third term in brackets becomes
\beq
\frac{\partial}{\partial \mathcal{L}_t} (\mathcal{L}_t \tilde{C},L \mathcal{L}_t \tilde{C}) = (\tilde{C},L \tilde{C}) \frac{\partial}{\partial \mathcal{L}_t} \mathcal{L}_t^2 = 2(\tilde{C},L\tilde{C}) \mathcal{L}_t.
\eeq
...
\beq
0 = -(\tilde{C},L\tilde{S}) - (\tilde{C},L\tilde{S}) - 2 \mathcal{L}_t (\tilde{C},L\tilde{C}) - 2 \sum_i c_i (\tilde{C},L\tilde{\phi}_i)
\eeq

\noindent Rearranging gives
\beq
-(\tilde{C},L\tilde{S}) = \mathcal{L}_t (\tilde{C},L\tilde{C}) + \sum_i c_i (\tilde{C},L\tilde{\phi}_i)
\label{eq:PdLambda}
\eeq

Now we perform the variation with respect to a general $c_k$ as in (\ref{eq:KohnStationary}).
\beq
0 = \frac{\partial \mathcal{L}_v}{\partial c_k} = -\Big[ (\tilde{S},L \tilde{\phi}_k) + \mathcal{L}_t (\tilde{C},L \tilde{\phi}_k) + (\tilde{\phi}_k,L \tilde{S}) + \mathcal{L}_t (\tilde{\phi}_k,L \tilde{C}) + \frac{\partial}{\partial c_k} (\sum_i c_i \tilde{\phi}_i, L \sum_j c_j \tilde{\phi}_j) \Big]
\label{eq:PdCk1}
\eeq
...
F =
\begin{bmatrix}
(\boldsymbol{\tilde{C},L\tilde{C}}) & (\boldsymbol{\tilde{C},L\tilde{\phi}}) & (\boldsymbol{\tilde{C},L\tilde{S}}) \\
(\boldsymbol{\tilde{\phi},L\tilde{C}}) & (\boldsymbol{\tilde{\phi},L\tilde{\phi}}) & (\boldsymbol{\tilde{\phi},L\tilde{S}}) \\
(\boldsymbol{\tilde{C},L\tilde{S}}) & (\boldsymbol{\tilde{S},L\tilde{\phi}}) & (\boldsymbol{\tilde{S},L\tilde{S}})
\end{bmatrix},
\eeq
...
where
\beq
\boldsymbol{B^T X} = \mathcal{L}_t (\tilde{C},L\tilde{S}) + \sum_i c_i (\tilde{\phi}_i, L\tilde{S}).
\eeq

\noindent A more compact way of writing equation \ref{eq:GenDFDT2} is by
\beq
\mathcal{L}_v = -\left( \Psi^{t,0},L \tilde{S} \right).
\eeq
...
\beq
(\tilde{S},L\tilde{C}) - (\tilde{C},L\tilde{S}) = (\bar{S},L\bar{C}) - (\bar{C},L\bar{S}).
\eeq

\noindent This can finally be written as the general form of equation \ref{eq:SLCandCLS}, giving
\beq
(\tilde{S},L\tilde{C}) = (\tilde{C},L\tilde{S}) + 1.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=6in]{swave-comparisons}
...
\textbf{@TODO:} Refer back to the Bransden and Joachain book and/or the Ray paper about decomposing this equation into the different parts.  Talk about what type of resonances these are (Feshbach?).  Refer to the discussions in Peter's paper and Yan/Ho's paper about the resonances corresponding to 2S and 3S.  Discussion on energy levels elsewhere for H and Ps, along with what the threshold is.  
...
Close coupling (9Ps9H \emph{et al} 2002) \cite{Blackwood2002} & $4.37$ & $0.10$ & --- & --- \\
Close coupling (22Ps1H + H$^-$ \emph{et al} 2002) \cite{Blackwood2002b} & $4.141$ & $0.071$ & $4.963$ & $0.033$ \\
