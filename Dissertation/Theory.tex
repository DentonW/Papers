...
\beq
\delta I_\ell = I_\ell[\Psi_\ell^t] - I_\ell[\Psi_\ell] \sim (L_\ell^t - L_\ell) \Det{\textbf{u}}.
\eeq
...
\begin{equation}
  ...
  \right]^{m_\ell}.
\end{equation}
From \cref{tab:LCList}, the derivatives $f_\ell^\prime(\rho)$ and $f_\ell^{\prime\prime}(\rho)$ are needed. In the \emph{Mathematica} notebook ``General Shielding Function.nb'' \cite{Wiki}, I found out that the derivatives can be written generally as
\beq
