...
We present briefly the numerical techniques below. Further details
...
method using the MATLAB \cite{MATLAB} nonlinear fitting routine 
...
Our calculation yields a better value for this quantity than the earlier 
...
For unpolarized Ps and H beams,
the singlet cross sections for each of the partial waves contribute $1/4$
to the integrated, differential, and momentum transfer cross sections, while 
the triplet cross sections contribute $3/4$ \cite{Joachain1979,Blackwood2002,Ray1996}. 
...
differential cross section is essentially
...
Figs.~\ref{fig:diff-cross-section-2D-kappa},
...
shows a slight increase in the backward direction while a small dip at 0.25 eV
is seen in the forward direction. At approximately 0.46 eV, the backward scattering
is suppressed, and there is a very rapid rise in the forward direction. From
...
For very low energies ($E_{\bm \kappa} < 10^{-6}$ eV),
...
After 0 energy up to approximately 0.46 eV, $\sigma_m > \sigma_{el}$, indicating scattering
...
the second arctan term, as there is only a single resonance for each
...
similar results to that of Figure 5 of the previous Kohn paper
\cite{VanReeth2003}. In that paper, they noted that the singlet result lie on a
...
tried this expansion out to a $\kappa^3$ term but did not notice a
