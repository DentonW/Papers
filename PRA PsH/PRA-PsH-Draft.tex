...
$^1F$-wave. \hl{The differential cross section exhibits interesting features,
including a change from slightly backward peaked to
strongly forward peaked scattering as $E_{\bm \kappa}$ increases and
rich structure due to multiple resonances near the Ps(n=2) threshold.}
Finally, we give a detailed analysis of the scattering lengths
...
difficulty of \hl{creating} an H beam, although the binding energy of positronium
...
correlations are \hl{important}. We present
\cite{Conferences1,Conferences2,Conferences3,WoodsDiss2015} 
our work of the application of the complex Kohn variational 
method to elastic Ps(1s)-H(1s) scattering for the \hl{energy range}
up to the excitation threshold of Ps(n=2) at 5.101 eV.
...
extend their $^{1,3}S$ and $^{1,3}P$ Kohn variational calculations in 
multiple ways. In addition to \sout{implementing}
the Kohn and inverse Kohn variational methods, we implement the 
generalized Kohn \hl{method}, the complex Kohn \hl{methods} for the $S$ and $T$ matrices, and the
generalized complex Kohn \hl{method} for the $S$ and $T$
...
performed a variational calculation with a Hylleraas basis set, and Yan and Ho
...
\cite{Drachman1975,Drachman1976} \hl{used}
...
14Ps14H, \hl{was used} for singlet \hl{scattering} only and included 14 eigen- and pseudo-states of 
Ps and also of H. Good agreement \hl{was} obtained between the CC
...
channel in a 22Ps1H coupling scheme, comparing \hl{with} the previous 22Ps1H
...
Kohn variational results \cite{VanReeth2003}. They speculated that the
...
%\todoi{Check Cooper paper}
\hl{The Kohn variational method gives rigorous upper bounds on the scattering lengths
and, except for Schwartz singularities, empirical lower bounds on the elastic
...
benchmark results \cite{VanReeth2003,VanReeth2004} \hl{with} which results from 
...
with ground-state 
H, Ps(1s)+H(1s), for incident energies up to the excitation threshold 
...
variational method for e$^+$-He scattering. While the results we present in Sec.
...
have a single formulation of the Kohn variational method and its variants.
...
\end{equation}
The scattering wavefunctions contain both the long-range terms $\bar{S}_\ell$
and $\bar{C}_\ell$ and the short-range correlation terms $\bar{\phi}_{i\ell k}$
for small interparticle distances. The 
...
vector of the center of mass of the Ps atom with respect to the proton\hl{,
...
$P_{23}$ is the exchange operator for the two indistinguishable electrons\hl{, and}
...
\end{smallmatrix} \right]\text{\sout{, and}} \label{eq:GenTKohn} \\
...
\end{align}
\end{subequations}
%\todoi{S-matrix when describing Kohn?}
...
Kohn \textbf{u} matrix is identical to theirs, and the generalized complex Kohn
\textbf{u} matrix is similar.
...
code has shown 
that these mixed symmetry terms can be important for that 
system. This investigation \cite{VanReeth2015} found that
...
compensated for the \hl{lack of convergence due to the
error} in the previous inclusion of the mixed symmetry
terms. The final numerical results used in \cite{VanReeth1997} are
...
\end{equation}
where $E_H$ and $E_{Ps}$ are the \hl{ground-state} energies of H and Ps, 
respectively,
...
K_\ell = \tan \delta_\ell = (u_{01} + u_{11} L_\ell)(u_{00} + u_{10}
  L_\ell)^{-1},
\end{equation}
\hl{reintroducing the subscript $\ell$.}
...
\hl{Using the first term, $S_\ell$, in the wavefunction,}
Eqs. (\ref{eq:TrialWave}) and (\ref{eq:TrialWaveHigher}), gives for the first
Born approximation (B1) to $\tan\delta_\ell$ \cite{Bransden2003}
\begin{equation}
\label{eq:Born}
...
\end{equation}
We consider the approximation with \hl{very} small $\kappa$ of
\begin{equation}
...
effective range is defined for \hl{only} the $S$-wave \cite{Levy1963}.
...
\end{equation}
We use the van der Waals coefficient, $C_6$, of 34.78473 a.u. as given
...
We present briefly the numerical techniques below. \hl{Further details}
can be found in Ref.~\cite{WoodsDiss2015}.

\subsection{Short-Short Integrations}
\label{sec:ShortInt}
For the \hl{$^1S$ PsH} bound state and $^{1,3}S$ Ps(1s)-H(1s) elastic scattering
calculations, we use
...
\end{align}
\end{widetext}
\hl{We solve these integrals using two different procedures.}
We use the first procedure, which is given by Van Reeth \cite{VanReethThesis},
...
This first procedure requires separate derivations and codes
for each partial wave. The second procedure is from Yan and Drake \cite{Yan1997}
\hl{and works for arbitrary $\ell$, requiring only a single codebase.
We present results using this second procedure for only $\ell > 2$ due
...
them unimportant. We find that this \hl{improves} the convergence of the matrix 
elements.

To further improve the convergence of the short-range--long-range matrix 
elements, we note that the biggest source of difficulty \sout{in converging these}
...
points to more than seven times as many \hl{as} in previous work
...
use more short-range terms, reducing linear dependence.
...
\begin{figure}[H]
	...
	\caption{(Color online) \hl{Breakdown in convergence} of the $^1S$ phase
shifts with respect to number of short-range terms for different $\tau$
values for the generalized Kohn}
	\label{fig:swave-phase-divergence}
\end{figure}

%\todoi{Make sure not describing tables more than once}
...
$^2\Gamma$) of the two resonances. This formula \hl{comprises} the
...
We fit the data from each variant of the Kohn variational
\hl{method} using the MATLAB \cite{MATLAB} nonlinear fitting routine 
\texttt{nlinfit} with all eight possible weightings. For each variant of the 
Kohn variational method we use (Kohn, inverse Kohn, complex $T$ and $S$ matrix,
...
use two different sets of these nonlinear parameters. One set gives higher
...
distances. The Rayleigh-Ritz variational method provides a true \hl{upper}
bound on the 
...
Our calculation yields a better value for \hl{this quantity} than the earlier 
...
used Hylleraas-type wavefunctions. While we do not obtain the best value of the 
binding energy (this is not the purpose of our calculation), the result
...
%\todoi{Make sure that it is clear that we have better phase shifts, even if not higher.}
...
%\todoi{She has a change here}
\hl{In general,} because of the 
...
$^{1,3}P$-wave \hl{indicate} that the complex Kohn phase shifts are well converged.
...
Ref.~\cite{Blackwood2002} notes that this shows that the interaction
...
The percentage difference between the $^3D$ $\omega = 6$ and extrapolated
...
The complex Kohn $^{1,3}D$ phase shifts are below the corresponding
...
higher $\kappa$ (not including the resonance region of $\kappa > 0.7$ or $E_{\bm \kappa} > 3.3$ eV).
...
integrated cross section shown in Fig. \ref{fig:combined-cross-sections} is
...
spin-weighted elastic integrated cross section, respectively. In the full
energy range, including the resonance region, the $^3D$ contributes a maximum of
...
$^{1,3}D$ phase shifts (less than $0.084\%$). The triplet $D$-, $F$-, $G$-, and
...
complex Kohn phase shifts with the Born approximation phase shifts.}
...
\hl{from} positive to negative \sout{present} for the $^3F$ phase shifts, but this change 
...
than $50\%$, however the triplet contributes much less to the total cross sections.

The modified Born phase shifts are approximately equivalent to the Born 
results, so we show only the Born results in this figure.
...
Kohn phase shifts for the \hl{$^{1,3}G$-wave} and $^{1,3}H$-wave. Thus, in 
computing the elastic integrated cross sections, we would not be comfortable 
matching on \hl{to} the Born approximation results for $\ell \geq 5$.
...
$^{1,3}G$, and $^{1,3}H$ partial waves are not fully converged, but for each of
...
section is $0.009\%$ and much less at energies before the threshold.
...
\hl{For unpolarized Ps and H beams,}
the singlet \hl{cross sections} for each of the partial waves contribute $1/4$
to the integrated, differential, and momentum transfer cross sections, while 
the triplet \hl{cross sections} contribute $3/4$ \cite{Joachain1979,Blackwood2002,Ray1996}. 
...
monotonically. The singlet cross section \sout{contribution} not only has resonance
...
and \ref{fig:combined-diff-cross-sections}. The percent difference \hl{by} including
...
are relatively well converged. As expected, 
Fig.~\ref{fig:diff-cross-section-2D-kappa} shows that the 
differential cross section is \hl{essentially}
isotropic at \hl{very low} incident energy and becomes slightly more backward peaked 
as the energy is increased up to about \mbox{0.46 eV} ($\kappa = 0.26$). 
...
\hl{Figs.}~\ref{fig:diff-cross-section-2D-kappa},
\ref{fig:diff-cross-section-2D-theta}, and 
...
The differential cross section at very low energy starts off isotropic and then
shows a slight increase in the backward direction while a small dip at 0.2 eV
is seen in the forward direction. At approximately 0.4 eV, the backward scattering
is suppressed and there is a very rapid rise in the forward direction, and from
there onward, we can see that most scattering occurs between 0 and 1 radians.
...
%\begin{figure}[H]
	...
	%\caption{(Color online) The elastic differential cross section for Ps-H scattering}
	%\label{fig:combined-diff-cross-sections}
%\end{figure}
%
%\begin{figure}[H]
	%\centering
	%\includegraphics[width=3.3in]{combined-diff-cross-sections-2}
	%\caption{(Color online) \hl{The elastic differential cross section for Ps-H scattering showing the change
%from backward peaked to forward peaked as $E_{\bm \kappa}$ increases}}
	%\label{fig:combined-diff-cross-sections-2}
%\end{figure}
%
%%\begin{figure}[H]%
    %%\centering
    %%\subfloat[label 1]{{\includegraphics[width=3.3in]{combined-diff-cross-sections-2} }}%
    %%%\qquad
    %%\subfloat[label 2]{{\includegraphics[width=3.3in]{combined-diff-cross-sections} }}%
...
\begin{figure}[H]%
    ...
    \caption{(Color online) The elastic differential cross section for Ps-H scattering at two different viewing angles}%
    \label{fig:combined-diff-cross-sections}%
\end{figure}
...
\begin{figure}[H]
	...
	\caption{\hl{(Color online) Comparison of cross sections. The complex Kohn integrated
...
Static exchange $\sigma_m$ data (light blue / light gray dash-dot) 
...
cross section only being \hl{essentially} isotropic at very low energy
(see Fig.~\ref{fig:diff-cross-section-2D-kappa}).
\hl{After 0 energy up to approximately 0.46 eV, $\sigma_m > \sigma_{el}$, indicating scattering
primarily in the backward direction, as also seen in}
Fig.~\ref{fig:diff-cross-section-2D-theta}. \hl{Above 0.5 eV, the scattering
...
The $^1S$ and $^1P$ partial waves \hl{each} have two resonances \hl{below} the Ps(n=2) 
threshold, and the $^1D$ has one resonance before. There is a resonance just 
\hl{above} the threshold for $^1F$, with the onset of the resonance \hl{obvious below} the 
threshold. Drachman \cite{Drachman1979} \hl{concluded} that these Rydberg 
resonances correspond to the quasibound state of e$^+$ with the H$^-$ ion.
Figure \ref{fig:spd-wave-phases}(a) shows the 
two $^1S$ Rydberg resonances below the Ps(n=2) \hl{threshold}. The first resonance 
 was first 
calculated by Hazi and Taylor using a stabilization method \cite{Hazi1970}. 
\hl{Their properties} have been computed accurately by Yan and Ho using the complex 
...
the second arctan term, \hl{as there is only a single resonance for each
...
complex Kohn resonance parameters with results from other calculations. \sout{The
...
Excellent agreement between the parameters is achieved between the present
complex Kohn and the earlier Kohn/inverse Kohn calculations 
\cite{VanReeth2003,VanReeth2004}. Good agreement is achieved 
...
work using the $S$ matrix complex Kohn. There is less \hl{good} agreement with the
...
We observe no \hl{triplet} resonances for any of these partial waves, which 
...
relatively well, but they could not obtain a resonance position as accurately as \hl{when}
performing a full scattering calculation. We use the same technique for the $^1S$,
...
Eq.~(\ref{eq:ScatLenApprox}) to \hl{determine}
the $^{1,3}S$ scattering lengths from the phase shifts of very
small $\kappa$, given in Table \ref{tab:SWaveScatLenERT}. %The $^{1,3}S$
...
ranges from these previous calculations %\sout{from prior Kohn variational \cite{VanReeth2003} and CC
%\cite{Blackwood2002,Walters2004} calculations}.
Some other recent elaborate
...
$\kappa = 0-0.5$ range for both \sout{the} singlet and triplet \hl{scattering}. In the third line of
...
similar \hl{results} to that of Figure 5 of the previous Kohn paper
\cite{VanReeth2003}. In that paper, they noted that the singlet \hl{result lie on} a
relatively straight line, but the triplet \hl{result} curves down at low energies. We see
...
tried this expansion \hl{out to a} $\kappa^3$ term but did not notice a
...
We also investigate this low $\kappa$ range using the QDT
...
Eq.~(\ref{eq:EffectiveRangeLongAu}). The scattering lengths 
\hl{we obtain} with the QDT expansion agree with results using the other 
...
the $^{1,3}P$-wave does not have effective ranges but does have scattering lengths 
...
\section{Conclusion}
We have presented complex Kohn variational results for Ps(1s) scattering from H(1s)
below the Ps(n=2) threshold. We have extended the earlier Kohn variational 
calculations \cite{VanReeth2003,VanReeth2004} with a larger basis set, which 
is made possible by Todd's procedure, the asymptotic expansion method,
an increase in the number of integration points and, for
$^{1,3}D$, the introduction and subsequent removal of the $e^{-\lambda r_i}$
term to the Gauss-Laguerre quadratures. With these improved numerics, we have
been able to obtain better converged phase shifts.
We have also used the $S$ matrix complex Kohn variational method, which also allows
...
We determine highly accurate $^{1,3}S$ and $^{1,3}P$ phase shifts.
...
cross section is small, and the $^1D$ resonance \hl{we compute} with the complex Kohn 
...
system at short distances. The $^{1,3}F$, $^{1,3}G$, and $^{1,3}H$ partial
waves have very small phase shifts and do not contribute greatly to the
elastic integrated, or momentum transfer cross
sections.
...
\hl{The elastic differential cross section is slightly more backward peaked at
...
$^1F$ partial waves, which compare favorably \hl{with} the complex 
...
and are likely \hl{to be} the most accurate to 
...
values, we obtain \sout{a $^3S$ effective range} \hl{a noticeably larger result}.
...
%In summary, using the $S$ matrix complex Kohn for elastic Ps-H 
%scattering, we calculate phase shifts through the $^{1,3}H$-wave and
%obtain highly accurate $^{1,3}S$ and $^{1,3}P$ phase shifts and 
%accurate elastic integrated, momentum transfer and ortho-para 
%conversion cross sections.
%We also compute the resonance parameters through
%$^1F$ and investigate the effective ranges for $^{1,3}S$ and the scattering 
%lengths for $^{1,3}S$ and $^{1,3}P$.


\begin{acknowledgments} We wish to thank Drs. Y.~K.~Ho,
J.~W.~Humberston, K.~Pachucki and Z.-C.~Yan for discussions.
We appreciate communications with Drs. Edward Armour, James 
Cooper and Martin Plummer, and files from Dr. Allan Todd. S.~J.~W. acknowledges 
support from NSF under grant no. PHYS-0968638 and from UNT through the UNT 
faculty research grant GA9150. Computational resources were provided by UNT's 
High Performance Computing Services, a project of Academic Computing and User 
Services division of the University Information Technology with additional 
support from UNT Office of Research and Economic Development.
\end{acknowledgments}
