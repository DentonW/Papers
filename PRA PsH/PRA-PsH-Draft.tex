...
\todoi{Do we need to explicitly write out the Hamiltonian? We could just refer to the J. Phys. B paper.}

The variable $\rho$ describes the position of the center of mass of the positronium atom, i.e. $\vec{\rho} = \frac{1}{2}\left(\vec{r_1} + \vec{r_2}\right)$.
...
\todoi{We do have a generalized complex Kohn method for the T-matrix, including the $\tau$ terms. Should we show the derivation with that instead?}

Previous work on Ps-H scattering \cite{??} used the Kohn and Inverse Kohn approach. However, both of these methods are known to be sometimes affected by Schwartz singularities. We therefore decided to also calculated the phaseshift using the generalized complex Kohn method, of which the Kohn and inverse Kohn are two real cases. \todo{Not true}
...
\beq
\Psi_l^t = \bar{S} + T_t \, \bar{W} + \sum_{i=1}^N c_i \bar{\phi_i}^t,
...
$\bar{S}$, $\bar{C}$ and $\bar{\phi_i}$ are the same as in (\ref{SCphiBarDef}).  Note that this is different than Cooper et al. \cite{Cooper2010}, in that we do not use a variable $\tau$, and our $\bar{W}$ is their $\bar{T}$ but with $\bar{C}$ and $\bar{S}$ swapped, which gives a wavefunction of the correct form \todo{include parentheses?} (an outgoing wave).
...
\eeq
\todoi{Why do some of the Kohn methods have a $\kappa$ included?}
...
\todoi{!!! NEED TO SEE WHICH EQS FROM 3.73 to 3.79 ARE TO BE KEPT}


Using equations \ref{eq:TrialComplex} and \ref{eq:ComplexKohnVariation}, the stationary property in equation \ref{eq:ComplexKohnStationary} can be written in matrix form as
...
\subsection{Complex Kohn Variational Principle (S-Matrix)}
...
Similar to the treatment in \cite{Cooper2010}, the complex-valued trial wavefunction is
\beq
\Psi_l^t = \tilde{S} + S_t \, \tilde{W} + \sum_{i=1}^N c_i \bar{\phi_i}^t,
\label{eq:TrialComplexS}
\eeq

where
\beq
\tilde{S} = \bar{C} - \ii \bar{S} \text{ and } \tilde{C} = \frac{1}{2\ii}(\bar{C} + \ii \bar{S}).
\label{eq:SCTildeDef}
\eeq
\todoi{Is this the correct form? My document had 3 versions of the derivation. Do we want to include the $\frac{1}{2\ii}$ in $\Psi_l^t$ above?}

$\bar{S}$, $\bar{C}$ and $\bar{\phi_i}$ are the same as in (\ref{SCphiBarDef}).
...
The variation of $\Psi$ is
\todoi{Check all of these equations.}
\beq
\delta\Psi = \Psi^t - \Psi = (\tilde{S} + S_t \tilde{C}) - (\tilde{S} + S \tilde{C}) = (S_t - S) \bar{C} = \delta S \,\bar{C}
\eeq
...
or
\beq
I[\Psi_t] - I[\Psi] = I[\Psi_t] = 2\ii(S_t - S).
\eeq

\noindent Writing this as a variation gives
\beq
S_v = S_t - \frac{1}{2\ii} I[\Psi_t].
\label{eq:ComplexKohnVariation}
\eeq
...
We can use this now in the stationary property of the complex Kohn functional, i.e.
\beq
\frac{\partial S_v}{\partial S_t} = 0  \text{ and } \frac{\partial S_v}{\partial c_i} = 0 \text{ where $i = 1,\ldots,N$}.
\label{eq:ComplexKohnStationary}
\eeq
...
\todoi{We need to stick with a consistent notation, whether bra-ket or define the parentheses}
\begin{equation}
\label{eq:ComplexKohnMatrix}
\begin{bmatrix} 
 (\tilde{C},L\tilde{C}) & (\tilde{C},L\bar{\phi}_1) & \cdots & (\tilde{C},L\bar{\phi}_j) & \cdots\\
 (\bar{\phi}_1,L\tilde{C}) & (\bar{\phi}_1,L\bar{\phi}_1) & \cdots & (\bar{\phi}_1,L\bar{\phi}_j) & \cdots\\
 \vdots & \vdots & \ddots & \vdots \\
 (\bar{\phi}_i,L\tilde{C}) & (\bar{\phi}_i,L\bar{\phi}_1) & \cdots & (\bar{\phi}_i,L\bar{\phi}_j) & \cdots\\
 \vdots & \vdots & & \vdots & \\
...
\todoi{We have introduced $S_v$ here only}

\noindent Finally, we solve for $S_v$, from which we obtain our phase shifts.
\begin{equation}
S_v = -\frac{1}{2\ii} \left( \textbf{\emph{B}}^{tr} \textbf{\emph{X}} - (\bar{S},L \bar{S}) \right)
\end{equation}
\todoi{Should the $\frac{1}{2\ii}$ be here? Check equation.}
...
\noindent The shielding factor, $f_{sh}(\rho)$ ensure that the singularity of the Neumann function is removed the origin is given by
\beq
...
\noindent The short-range terms are given by and represent the angular momentum as been placed on either the positron ($r_1$) or on the electron on the Ps atom ($r_2$).
\begin{subequations}
...
\end{align}
\end{subequations}
where $\phi_i$ is given by equation \ref{eq:PhiDef}. For partial waves with $L > 1$, the angular momentum could be shared between both particles in the Ps atom \cite{??} (Schwartz). An investigation of the contribution to the final results from each symmetry  (find paper Pos-He 1999 or thesis) has revealed that the mixed symmetries were not important and because of the complexity of the anlaytical evaluation of the various matrix elements, we have omitted the mixed symmetry term. The formulation for the Complex Kohn method is the same as that expressed above.
\todoi{Mention the other form we tried here or later?}
...
In this equation, $r_1$ is the distance of the positron from the proton, $r_2$ is 
...
To be consistent with the scattering calculation, we report the results obtained with the same set of nonlinear parameters that were used in the scattering calculation, namely $\alpha = 0.586$, $\beta = 0.580$ and $\gamma = 1.093$. For our set with 1505 terms, we calculate a nonrelativistic ground-state energy of -0.789189725 au, giving a binding energy of 1.066406705 eV (using a conversion factor of 1 au = 27.21138505 eV from \cite{?}). We note that we are able to obtain a slightly better value for the binding energy with higher $\omega$, but we are unable to use this many terms in the full scattering calculations for Ps-H.
...
for the triplet, \todo{Run on?} although Ray obtained a triplet resonance
