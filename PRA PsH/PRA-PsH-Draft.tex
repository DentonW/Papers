...
$^1F$-wave. Finally, we give a detailed analysis of the scattering lengths
...
method to elastic Ps(1s)-H(1s) scattering for the energies up to the 
...
multiple ways. In addition to implementing
...
for the Ps-H scattering lengths. Refs. \cite{Drachman1975,Drachman1976} \hl{use
...
14Ps14H, is for the singlet only and included 14 eigen- and pseudo-states of 
...
\cite{Zhang2012} \hl{uses the
confined variational method (CVM) to calculate phase shifts for 2 momenta
for both $^1S$ and $^3S$.} This provides accurate
results but has the drawback of being very computationally expensive.

The Kohn variational method gives rigorous 
...
methods \cite{VanReeth2003, VanReeth2004}. \sout{The complex Kohn variational
...
\end{equation}
with all $k_i, l_i, m_i, n_i, q_i$ and \hl{$p_i \geq 0$ }\cite{VanReeth2004}.
...
atom ($r_3$) and on the Ps ($\rho$). \hl{Since we find that this did not improve the
...
A preliminary investigation \hl{for e$^+$-H} \cite{VanReeth2015} with a corrected
code has shown 
that these mixed symmetry terms can be important for that 
...
been found to make no significant contribution. \hl{The investigation of}
Ref.~\cite{VanReeth2015} \hl{has been extended to
...
\subsection{Derivation of Kohn Variational Method and Variants of the Method}
\label{sec:Kohn}
...
Using the stationary property of the functionals for the \hl{Kohn and variants}, we obtain
\begin{equation}
...
and reintroducing the subscript $\ell$
\begin{equation}
\label{eq:GenKohnL}
...
is given by
\sout{using the first term, $S_\ell$, in Eqs.~(\ref{eq:TrialWave}) and
...
\end{equation}
We consider the approximation with small $\kappa$ of
\begin{equation}
...
Eq.~(\ref{eq:GaoKTaylor}). \hl{We compute the $^{1,3}S$- and $^{1,3}P$ scattering
...
\hl{We present briefly the numerical techniques below. Details can be found in}
Ref.~\cite{WoodsDiss2015}.

\subsection{Short-Short Integrations}
\label{sec:ShortInt}
For the $^1$S PsH bound state and $^{1,3}S$ Ps(1s)-H(1s) elastic scattering
calculations, we use
...
$D$-wave, as the integrals become too singular for $\ell > 2$. \hl{The second is
a more general method from Yan and Drake} \cite{Yan1997}, \hl{which works for all
partial waves.}
...
\cite{VanReeth2003,VanReeth2004} to better represent the integrands. \hl{We use
a visual representation of the matrix elements to determine convergence using
the Developer's Image Library} \cite{DevIL}. \hl{The brute force approach
...
cause the difference to be greater than a predetermined threshold. \sout{We refer 
...
the results in Sec.~\ref{sec:Results}. \hl{For $^1S$ only, we determine
...
Eq. (\ref{eq:PhiDef}).
\hl{When we perform convergence checks via extrapolations, we
...
\hl{We fit the data from each variant of the Kohn variational}
methods using the MATLAB \cite{MATLAB} nonlinear fitting routine 
\texttt{nlinfit} with all eight possible weightings. \hl{For each variant of the 
...
bounds to the phase shifts, but they are found to give empirical bounds
...
length is considered the error in \hl{Tables} \ref{tab:SWaveScatLenERT} and
\ref{tab:PWaveScatLen}. \hl{We see no convergence pattern for the effective range.}
...
\hl{For $\ell \geq 2$, where we have neglected the mixed symmetry terms,
...
to two factors. Using Todd's procedure (described in Sec. \ref{sec:Truncation}) 
...
correspondingly small. Before the resonance region ($\kappa \leq 0.7$), the
...
energy range, including the resonance region, $^3D$ contributes a maximum of
...
$H$-wave phase shifts are more sensitive to the nonlinear parameters than the 
...
of positive to negative present for the $^3F$ phase shifts, but this change 
...
The modified Born is approximately equivalent to the Born results, so we show 
only the Born results in this graph. The Born approximation phase shifts does 
...
matching on the Born approximation results for $\ell \geq 5$.
...
these, the phase shifts and partial elastic cross 
...
\ref{tab:DWaveResonances}. \hl{There is also some discrepancy at low
$E_{\bm \kappa}$.}
...
and \ref{fig:combined-diff-cross-sections}. The percent difference of including
...
They have been computed accurately by Yan and Ho using the complex 
...
relatively well, but they could not obtain a resonance position as accurately as 
...
scattering, we obtain benchmark $^{1,3}S$ and $^{1,3}P$ phase shifts and 
accurate elastic differential, integrated, momentum transfer and ortho-para 
conversion cross sections. We also compute the resonance parameters through
...
Cooper and Martin Plummer, and files from Allan Todd. S.~J.~W. acknowledges 
