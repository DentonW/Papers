...
while the \textbf{u} matrix for the generalized complex Kohn
$T$ matrix is similar to their corresponding matrix.
...
are slightly higher than the CC phase shifts at both $\kappa = 0.6$ and 0.7.
...
section after zero energy, and as 
...
calculation. The CC results \cite{Walters2004}
are comparable to the complex Kohn and complex rotation calculations.
...
Some other calculations of the scattering lengths and
...
of S-wave scattering lengths and effective ranges can also be found in
...
Fig.~\ref{fig:swave-ERT-short}, but uses the complex Kohn phase shifts
...
to the energy range over which the fit is made.
...
and to the results we obtain using the ERT for short-range 
...
the QDT and the two ERT fits, Eq.~(\ref{eq:EffectiveRangeShort}) and
Eq.~(\ref{eq:EffectiveRangeLongAu}), for the range $\kappa = 0.001 - 0.009$ are
close.
...
agree well for both $^1P$ and $^3P$. The $^1P$ scattering length
obtained from the SVM \cite{Ivanov2002} is comparable to the scattering
lengths from the complex Kohn. However, the $^3P$ scattering lengths
from the complex Kohn differs significantly from the result obtained
by the SVM \cite{Ivanov2002}. This appears
mainly due to SVM $^3P$ phase shifts 
