...
investigation into e$^-$-H scattering} \cite{VanReeth2015} \hl{has
...
Eq. (\ref{eq:PhiDef}), as shown in Table \ref{tab:Nonlinear}.
When we do convergence checks via extrapolations, we
...
use two different sets of nonlinear parameters. \hlc[cyan]{One set gives higher
...
\hl{Ps-H scattering for $\ell \geq 2$} warrants investigation.
...
The percentage difference for $^3D$ for $\kappa = 0.1$ is very large,
...
the singlet, but in general, the triplet contribution to the elastic
differential and integrated cross sections are less than the corresponding
...
not agree well with the complex Kohn phase shifts, being much lower. \hl{We 
...
sections become very small, so they do not contribute much to the integrated 
...
phase shifts (on the order of $\lesssim 10^{-5}$) and possibly the neglect of
...
approach \cite{Walters2004}. \sout{The first $^1P$ Rydberg resonance is associated
with the $3p$ state, not the $2p$ state \cite{DiRienzi2002b}, while the $^1D$ 
resonance is associated with the $3d$ state \cite{DiRienzi2002a}.}
...
\cite{Ivanov2002}, \hl{the complex Kohn $^3P$ scattering length is much smaller.}

\hl{In summary, using the complex Kohn for the $S$ matrix for elastic Ps-H 
...
$^1F$ and investigate the effective ranges and scattering lengths for $^{1,3}S$
and $^{1,3}P$.}
