...
this system. \sout{Using the Hylleraas-type basis set, we also compute the binding 
...
\cite{Drake1995, Yan1997} \hl{to improve the accuracy of matrix elements
containing only short-range terms}. We have also significantly increased
...
calculation} \cite{VanReeth2015} that \hl{correctly
...
\hl{We observe that using the terms selected by the Todd procedure allows us to
...
the results in Sec.~\ref{sec:Results}. \hlc[cyan]{For $^1S$ only, we are able to determine
the cutoff by performing variations of $\mu$} \cite{WoodsDiss2015}.
\sout{The value of $\mu$ that gives the
...
\hl{For $\ell = 3$ at low $\kappa$}, we use a restricted set of short-range terms
where we eliminate terms \hl{with powers of} $r_3 \geq 2$ if $\omega \geq 3$,
...
(\ref{eq:TrialWaveHigher}) are indicated by $N^\prime(\omega)$ instead of
$N(\omega)$ to indicate that this is not the full set described after
Eq. (\ref{eq:PhiDef}), as shown in Tables \ref{tab:Nonlinear1} and
\ref{tab:Nonlinear2}. When we do convergence checks via extrapolations, we
put the terms back in the original ordering but with the set of
$N^\prime(\omega)$ terms.
...
be confident with extrapolated phase shifts. \hl{We also find that for very small
phase shifts ($\delta_\ell^\pm \lesssim 10^{-5}$), our code is not accurate
enough to give convergence ratios less than 1.}
...
%\begin{table}[H]
  %\centering
	...
	%$N^\prime(\omega)$ & 1505  & 1633  & 1000  & 1000  & 916 $(\kappa < 0.3)$   & 919 $(\kappa < 0.3)$   & 385 $(\kappa < 0.4)$ & 385 $(\kappa < 0.4)$  \\
	%$N^\prime(\omega)$ &   &   &   &   & 913 $(\kappa \geq 0.3)$   & 913 $(\kappa \geq 0.3)$  &  462 $(\kappa \geq 0.4)$ &  462 $(\kappa \geq 0.4)$ \\
...
use two different sets of nonlinear parameters. \hl{One set gives more
positive phase shifts for the lower $\kappa$ values, and the other set gives
more positive phase shifts for higher $\kappa$ values.}
...
$\approx 140\%$, but the} \hl{percentage difference for the range $0.2 - 0.7$
...
The \sout{$\omega = 6$} $^{1,3}D$ phase shifts are below the corresponding
...
$^{1,3}D$ phase shifts (less than $0.084\%$).\hl{The triplet $D$-, $F$-, $G$-,
and $H$-wave phase shifts are more sensitive to the nonlinear parameters than
the singlet, but the triplet contribution to the elastic differential and 
integrated cross sections are less than the corresponding singlet contribution.}
...
occurs at a higher energy of approximately $3.5$ eV. The $^1F$-wave has a 
resonance above the Ps(n=2) threshold, but the beginning of the resonance is 
evident in Fig. \ref{fig:fwave-phases}. \hl{The difference between the $^1F$ phase
shifts for $\omega = 4$ and $5$ is less than $10\%$ for $\kappa \geq 0.5$
and is as low as $2.8\%$ at $\kappa = 0.75$. The corresponding difference
for the $^3F$ phase shifts is greater than $50\%$.

The modified} Born is approximately 
equivalent to the Born results, so we show only the Born results in these 
graphs. The Born approximation is not particularly good, being much lower. 
This comparison is not shown for the first three partial waves, as there is 
very little agreement. \hl{We also find little agreement between the Born
approximation and the complex Kohn phase shifts for the $^{1,3}$G-wave and
$^{1,3}H$-wave.}
Thus, in computing the integrated elastic cross 
sections, we would not be comfortable matching on the Born results for
$\ell \geq 5$.

We perform full complex Kohn calculations on all first six partial waves, but 
we do more extensive calculations for the first three partial waves, \hl{as shown 
...
\hl{For the G- and H-waves, we obtain a convergence ratio $R'(5) > 1$ for
$\kappa \leq 0.3$ and $\kappa \leq 0.35$, respectively, due to the very small
phase shifts (on the order of $\lesssim 10^{-5}$). The convergence ratios
are less than 1 at higher $\kappa$, where there is a more significant
contribution to the elastic differential cross section.
The maximum $H$-wave contribution to the integrated elastic cross
...
and \ref{fig:combined-diff-cross-sections}. \hl{The percent difference of including
the $H$-wave in the differential cross section versus only including through
the $G$-wave is a maximum of $3.8\%$ in
the resonance region but only an average of $0.26\%$ throughout the full
$E$ and $\theta$ range, indicating the the differential cross sections
...
resonance \hl{is associated} with the $3d$ state \cite{DiRienzi2002a}.
...
date. \hl{When we use a $\kappa$ range of 0.1 to 0.5, we obtain a $^3S$ effective
range close to those previously reported}
\cite{VanReeth2003,Blackwood2002,Ivanov2002}, \hl{but when we use smaller $\kappa$
...
\hl{In summary, using the complex Kohn for the $S$ matrix for elastic Ps-H scattering,
we obtain benchmark $^{1,3}S$ and $^{1,3}P$ phase shifts
and accurate elastic differential, integrated, momentum transfer and ortho-para
conversion cross sections. We also compute the resonance parameters through
$^1F$ and investigate the effective ranges and scattering lengths for 
$^{1,3}S$ and $^{1,3}P$.}


\begin{acknowledgments} We wish to thank Drs. Y.~K.~Ho,
\hlc[cyan]{J.~W.~Humberston}, K.~Pachucki and \hl{Z.-C.~Yan} for discussions.
We appreciate communications with Drs. \hlc[cyan]{Edward Armour}, James 
