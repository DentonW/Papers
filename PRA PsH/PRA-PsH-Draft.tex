...
for the Ps-H scattering lengths. Drachman and Houston \cite{Drachman1975,Drachman1976}
...
for both $^1S$ and $^3S$. \hl{This method} provides accurate
...
We present briefly the numerical techniques below. Details can be found in
...
distances. \hl{The Rayleigh-Ritz variational method provides a true bound on the 
...
used \hl{Hylleraas-type wavefunctions}. While we do not obtain the best value of the 
binding energy (this is not the purpose of our calculation), \hl{ the result
...
Kohn phase shifts are slightly larger than the CC results. \hl{Because of the 
...
$^{1,3}P$-wave indicates that the complex Kohn phase shifts are well converged.
...
higher $\kappa$ \hl{(not including the resonance region of $\kappa > 0.7$ or $E_{\bm \kappa} > 3.3$ eV)}.
...
correspondingly small. Before the resonance region ($\kappa \leq 0.7$ or $E_{\bm \kappa} < 3.3$ eV),
...
\hl{$\alpha$, $\beta$, and $\gamma$} than the 
...
\hl{The modified Born phase shifts are approximately equivalent to the Born 
...
we do more \hl{elaborate} calculations for the first three partial waves, as shown 
...
these, the phase shifts and \hl{elastic partial} cross 
...
cross sections. 
\sout{The integrated cross sections, shown in Fig.~\ref{fig:combined-cross-sections},
...
\ref{tab:DWaveResonances}. \hl{There is also some noticeable discrepancy at low
...
similar plots to that of Figure 5 of the previous Kohn paper
...
\hl{We determine highly accurate $^{1,3}S$ and $^{1,3}P$ phase shifts.}
...
\hl{In summary, using the $S$ matrix complex Kohn for elastic Ps-H 
