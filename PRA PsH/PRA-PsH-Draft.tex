...
scattering is of interest in the study of solar processes \cite{Crannell1976}. \todoi{Look at paper she mentions if it discusses Ps formation}
...
\todoi{Needs to be clear: (1) What we have done/improved over earlier Kohn.  (2) What we are presenting.}

We present results in this paper obtained using the $S$ matrix complex Kohn
variational principle for Ps-H scattering. We confirm the 
...
complex rotation calculations (references to be given \todo{Give}). In addition, we 
...
\cite{Blackwood2002}.
\todoi{H$^-$ importance. All the CC of Walters' group here.}
...
A recent calculation of Ps-H scattering uses the confined variational 
...
Zhang and Yan calculate phase shifts for 2 momenta for both $^1S$ and $^3S$.
Other earlier calculations of Ps-H scattering are given in Refs.~\cite{Massey1954} 
and \cite{Hara1975}.
\todoi{Give what those methods are. Any others?}
...
Van Reeth and Humberston \cite{VanReeth1999} also used the complex Kohn 
variational method for e$^+$-He scattering. The results we present in Sec.
\ref{sec:Results} use the $S$ matrix complex Kohn method, but we present in
...
\begin{equation}
\label{eq:PhiDef}
\bar{\phi}_{ik} = \left(1 \pm P_{23}\right) Y_{\ell 0}(\theta_j,\phi_j)
...
\todoi{Her full page of notes here...}
...
nonlinear parameters using the Newton \cite{Yan1999}, Broyden
\cite{Sauer2006} and simplex methods \cite{GSL}. For the $^3S$ case
...
nonlinear parameters are used in the scattering problem. There is an 
...
chosen so that the phase shifts are the most positive. We give the choices
of all parameters in Table \ref{tab:Nonlinear}.
...
This leads to the Born approximation given by
\begin{equation}
\label{eq:Born}
...
to find an expression relating the phase shifts to a quantity he refers to as
...
\end{equation}
The Z functions in this equation are given in Gao's paper \cite{Gao1998}. The
phase shifts are fitted to Eq. (\ref{eq:GaoZEqn}) to determine $K_\ell^0(E)$ for each
$\kappa$ value. $K_\ell^0(E)$ must be expanded in a Taylor series as
\begin{equation}
\label{eq:GaoKTaylor}
K_\ell^0(E_{\bm \kappa}) = K_\ell^0(0) + {K_\ell^0}^\prime(0) E_{\bm \kappa}
  + \ldots.
\end{equation}
\todoi{Multiple things here}
From this, $K_\ell^0(0)$ and ${K_\ell^0}^\prime(0)$ are determined.
In Ref. \cite{Gao1998a},
...
\todoi{More}
For further details on numerical techniques, refer to Ref. \cite{WoodsDiss2015}.
...
We find that this second method is slower, but we use it mainly for
$\ell = 2 - 6$, since it works for arbitrary $\ell$. These integrals have
the form of
\begin{widetext}
\begin{align}
...
the results in Sec.~\ref{sec:Results}.
\begin{figure}[H]
	\centering
...
$\delta_0^+$ is plotted for one Kohn method with respect to $N$ for 
...
The $^1D$ has one resonance below the inelastic threshold, and $1^F$ has a
resonance past the threshold that has 
...
\todoi{Why only did smaller $\omega$ for higher partial waves for Ps-H scattering}
...
improved with the explicit inclusion of the mixed terms. We note, however,
that the
...
which have a percentage difference of over 30\%, often often much larger,
...
We notice no appreciable difference to the cross section 
...
threshold and much less at lower energies. For Fig. \ref{fig:combined-cross-
sections}, we extract the CC data of Ref. \cite{Walters2004} using the 
...
shifts using Eq. (\ref{eq:TotalCross}). There is good comparison with the 
...
elastic integrated and momentum transfer cross sections. As with the 
...
\ref{eq:EffectiveRangeLongAu}. The scattering 
...
using the approximation to the definition in Eq.~\ref{eq:ScatLenApprox} 
evaluated at $\kappa = 0.01$, and QDT expressions. The $^1P$ scattering 
