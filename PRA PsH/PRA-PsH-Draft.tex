...
Kohn calculation. Next, we have significantly increased the number of integration points for matrix elements that involve the long-range terms, along with implementing a method to accelerate the convergence of these integrals. We have also extended the calculations to the next four partial waves through the H-wave.

We have applied the complex Kohn for the \emph{S} matrix variational principle to the first six partial waves for Ps-H scattering and present results. We confirm the previously observed resonances for the first four partial waves and compare the positions and widths to that of the earlier Kohn, close coupling (CC) and complex rotation calculations (references to be given). In addition, we computed the scattering lengths and effective range using multiple effective range theories. We also used the short-range part of the full scattering wavefunction to compute the binding energy of positronium hydride (PsH). Comparing the binding energy with the most elaborate variational results gives an indication of the reliability of describing the Ps-H system at short-distances.
...
the Stochastic Variational Method (SVM) \cite{Ivanov2001,Ivanov2002}, close coupling \cite{Blackwood2002,Blackwood2002b,Walters2004}, and Kohn variational \cite{Page1976,VanReeth2003,VanReeth2004} methods have been applied.
...
\todoi{Need to rewrite to include \cite{Blackwood2002b}. This is around the same time as \cite{Blackwood2002}, but they look at including H$^-$.}
...
The most recent calculation of Ps-H scattering uses the confined variational method (CVM) \cite{Zhang2012}. This provides accurate results but has the drawback of being very computationally expensive. In Ref. \cite{Zhang2012}, they calculate phase shifts for 2 momenta for both $^1S$ and $^3S$.
...
\emph{Add \cite{Mitroy2002a} and \cite{Mitroy2007}?}
\emph{Add \cite{Laricchia2009}?}
...
\todoi{Put something about \cite{VanReeth1999} in here? Peter used the complex Kohn for e$^+$-He.}
...
We also calculate the momentum transfer cross sections, given by \citep[p. 589]{Bransden2003}
\begin{equation}
...
\end{equation}
The Kohn variational methods give an exact upper bound on the scattering length \cite{Joachain1979}. The scattering length can give information about whether there is a bound state in the system. \todo{Blackwood eq?}
...
\begin{table}[H]
  \centering
    ...
    Parameter & $^1$S & $^3$S & $^1$P & $^3$P & $^1$D & $^3$D & $^{1,3}$F & $^{1,3}$G & $^{1,3}$H \\
    \colrule
...
\end{tabular}
\end{ruledtabular}
\caption{Positronium hydride energy comparisons. The values marked by an asterisk are the reported values, and the other values are obtained by using the conversion factor given in Ref. \cite{Mohr2012}.}
...
The recent CVM S-wave results from Zhang and Yan \cite{Zhang2012} agree extremely well with the Kohn results, even for the triplet. Figure \ref{fig:spd-wave-phases}(a) shows the full range of our calculations, with comparisons to the CC and CVM results. Excellent agreement between the three sets of results is evident. In Fig. \ref{fig:swave-comparisons}, we compare the $^{1,3}$S phase shifts obtained from the complex Kohn variational method with various other calculations.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=3.4in]{swave-comparisons}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=3.4in]{cross-section-comparisons}
	\caption{Comparison of cross sections}
	\label{fig:cross-section-comparisons}
\end{figure}

As pointed out in Ref. \cite{Buckman1989}, $\sigma_m = \sigma_{el}$ for zero energy. From Fig. \ref{fig:cross-section-comparisons}, we can see that this is the case. For very low energies ($E < 10^{-6}$ eV), $\sigma_m = \sigma_{el} = 32.45$. The ortho-para conversion cross section, $\sigma_c$, is much lower than the elastic and momentum transfer cross sections. Both the elastic and ortho-para conversion cross sections can be compared to that of Ref. \cite{Hara1975}. To the best of our knowledge, the momentum transfer cross sections have not been computed for this system before.
...
Current work \emph{S} matrix & $4.0065$ & $0.0955$ & $5.0277$ & $0.0607$ \\
...
Current work \emph{S} matrix & $4.2856$ & $0.0445$ & $5.0579$ & $0.0459$ \\
...
Current work \emph{S} matrix & $4.7186$ & $0.0864$ \\
...
Current work \emph{S} matrix $(\omega = 6)$ & $5.1863$ & $0.0125$ \\
...
Similar to the work by Van Reeth and Humberston on $^{1,3}$S \cite{VanReeth2003}, we use Eq. (\ref{eq:ScatLen}) to find the $^{1,3}$S and $^{1,3}$P scattering lengths for small values of $\kappa$, given in Table \ref{tab:SWaveScatLenERT}. For the P-wave, we were unable to use the very low $\kappa = 0.0001$ due to numerical inaccuracies. We see that to the accuracy reported, the $^{1,3}$S scattering lengths have converged well with respect to $\kappa$.

Prior work in the literature for Ps-H scattering \cite{Blackwood2002,Walters2004,VanReeth2003} uses the ERT expansion for short-range interactions in Eq. (\ref{eq:EffectiveRangeShort}). Table \ref{tab:SWaveScatLenERT} shows the S-wave scattering lengths and effective ranges from prior Kohn variational \cite{VanReeth2003} and CC \cite{Blackwood2002,Walters2004} calculations. Some other recent elaborate calculations of the scattering length and effective range can be found in Refs. \cite{Sinha2000}, \cite{Chiesa2002}, and \cite{Ivanov2002}. These all agree relatively well with each other.
...
When we do the fitting only at very small momenta ($\kappa = 0.001 - 0.009$ in Table \ref{tab:SWaveScatLenERT}) and compare to the higher momentum ($\kappa = 0.1 - 0.5$), we see that the singlet results stay nearly the same. However, for the triplet, the effective range changes significantly. This corresponds with the triplet curving downward in Fig. \ref{fig:swave-ERT-short}. The question as to whether this is a real feature or numerical inaccuracy at low momenta can be answered by looking at the convergence of the phase shifts at these $\kappa$ values. As Blackwood et al. note \cite{Blackwood2002}, numerical accuracy is difficult to obtain at very small $\kappa$. We estimate that for $\kappa = 0.001 - 0.009$, based off extrapolations, the $^1$S and $^3$S complex Kohn variational phase shifts have converged to within $0.28\%$ and $0.37\%$ of their correct values, respectively.
...
\todoi{Some discussion on the P-wave}
...
Data files with current results are available upon request from Denton Woods. These contain all discussed partial waves using all Kohn methods.

extended previous Kohn calculations
...
\todoi{From \cite{Ho1998}: ``Resonances in positronium-hydrogen scattering are the result of the Rydberg series of a positron attaching to a negative hydrogen ion, H− (Drachman 1979).''}
