...
\begin{abstract}
In this paper, we investigate the four-body Coulomb process of low-energy positronium-hydrogen (Ps-H) scattering below the Ps(n=2) excitation threshold with a Hylleraas-type basis set using the complex Kohn variational method. We provide benchmark phase shifts for the $^{1,3}$S- and $^{1,3}$P-wave and show phase shifts through the $^{1,3}$H-wave. Our results compare well to a number of other calculations for this system, including the binding energy of positronium hydride (PsH). We describe numerical methods used for calculating the binding energy and phase shifts. In addition, we present elastic differential, elastic integrated, momentum transfer, and ortho-para conversion cross sections, along with resonances through the F-wave. Finally, we give a detailed analysis of the scattering length and effective range using multiple effective range theories.
\end{abstract}
...
The low-energy region is of particular interest, because in this energy range, positron and electron correlations are dominant. We are presenting \cite{Conferences} in this paper our work of the application of the complex Kohn variational method to elastic Ps(1s)-H(1s) scattering in the energies up to the excitation threshold of Ps(n=2) at 5.101 eV.
...
Next, we have significantly increased the number of integration points for matrix elements that involve the long-range terms, along with implementing a method to accelerate the convergence of these integrals. We also extended the calculations to the next four partial waves through to the H-wave. We also calculate the differential elastic, integrated elastic, momentum transfer and ortho-para conversion cross sections, and we analyze the resonances through the F-wave.
...
Phase shifts are expressed in radians. Atomic units are used throughout unless otherwise stated. For conversions to electron-volts (eV), we use the conversion factor \mbox{1 au = {27.21138505(60) eV}} \cite{Mohr2012,*NISTConversions}.
...
\end{equation}
where $E_H$ and $E_{Ps}$ are the ground state energies of H and Ps, respectively. Notice that the wavefunction is not conjugated, as pointed out by Cooper et al \cite{Cooper2010}.
...
From Eq. (\ref{eq:ScatLenCross}), $\sigma_m = \sigma_{el}$ for zero energy. From Fig. \ref{fig:cross-section-comparisons}, we can see that this is the case. For very low energies ($E_{\bm \kappa} < 10^{-6}$ eV), $\sigma_m = \sigma_{el} = 32.45$. The momentum transfer cross section differs from the integrated elastic cross section past very low energies, and as noted in Ref. \cite{Blackwood2002c}, this is due to the differential elastic cross section only being isotropic at very low energy.

The ortho-para conversion cross section, $\sigma_c$, is much lower than the elastic and momentum transfer cross sections. Similar to the elastic integrated cross sections, the CC $\sigma_c$ curve is approximately the same as ours, but the resonances are clearly shifted. The static exchange $\sigma_m$ curve does not agree well with the complex Kohn results, but the static exchange is an earlier calculation that does not predict resonances.
