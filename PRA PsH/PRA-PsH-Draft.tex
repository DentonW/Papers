...
Kohn variational calculations \cite{VanReeth2003, VanReeth2004}.
...
elements that involve the long-range terms, along with implementing a procedure to
...
We present results in this paper obtained generally using the $S$-matrix complex Kohn 
...
variational method and any of its variants,
...
&\text{generalized $T$-matrix Kohn, } L^{\pm,t}_\ell = T_\ell^{\pm},
 \textbf{u} = \left[ \begin{smallmatrix}
\cos\tau & \ \sin\tau \\ -\sin\tau + \ii \cos\tau & \ \cos\tau + \ii \sin\tau
\end{smallmatrix} \right] \label{eq:GenTKohn} \\
&\text{generalized $S$-matrix Kohn, } L^{\pm,t}_\ell = S_\ell^{\pm},
 \textbf{u} = \left[ \begin{smallmatrix}
...
while the generalized complex Kohn
\textbf{u} matrix is similar to their corresponding matrix.
...
\texttt{nlinfit} with all eight possible weightings. For the Kohn and
each variant we use (inverse Kohn, generalized Kohn, complex Kohn for the $T$ 
matrix and complex Kohn for the $S$ matrix),
we determine and compare the four parameters for each
of the eight fits.
...
$^{1,3}D$ phase shifts (less than $0.084\%$). %\todo{check \%}
...
$^{1,3}G$, and $^{1,3}H$ partial waves are not converged, but for each of
...
Equations for the momentum transfer cross section are given
...
Good agreement between the parameters is achieved between the present
complex Kohn and the earlier Kohn/inverse Kohn calculations 
\cite{VanReeth2003,VanReeth2004}. Also, good agreement is achieved 
...
\end{tabular}
\end{ruledtabular}
\caption{$^1S$ resonance parameters for Ps-H scattering. The first line has the average
of the resonance parameters due to the Kohn variational method and variants
(inverse Kohn, generalized Kohn, complex Kohn for the $T$ matrix and complex Kohn for the
$S$ matrix) after Schwartz singularities are removed, and the errors are given by the standard deviation.}
...
\end{tabular}
\end{ruledtabular}
\caption{$^1P$ resonance parameters for Ps-H scattering. The first line has the average
of the resonance parameters due to the Kohn variational method and variants
(inverse Kohn, generalized Kohn, complex Kohn for the $T$ matrix and complex Kohn for the
$S$ matrix) after Schwartz singularities are removed, and the errors are given by the standard deviation.}
...
\end{tabular}
\end{ruledtabular}
\caption{$^1D$ resonance parameters for Ps-H scattering. The first line has the average
of the resonance parameters due to the Kohn variational method and variants
(inverse Kohn, generalized Kohn, complex Kohn for the $T$ matrix and complex Kohn for the
$S$ matrix) after Schwartz singularities are removed, and the errors are given by the standard deviation.}
...
\end{tabular}
\end{ruledtabular}
\caption{$^1F$ resonance parameters for Ps-H scattering. The first line has the average
of the resonance parameters due to the Kohn variational method and variants
(inverse Kohn, generalized Kohn, complex Kohn for the $T$ matrix and complex Kohn for the
$S$ matrix) after Schwartz singularities are removed, and the errors are given by the standard deviation.}
...
similar results to that of Fig. 5 of the previous Kohn paper
...
added a $\kappa^3$ term to Eq.~(\ref{eq:EffectiveRangeShort}) but noticed
no signficant difference in the quality of the fit from using
Eq. (\ref{eq:EffectiveRangeShort}) for the $^1S$.
However, for the $^3S$, they found that the addition of this term improved the fit,
...
We also apply using the QDT for the 
van der Waals interaction
of Gao \cite{Gao1998} at $\kappa$ = 0.002 and 0.003.
In Table \ref{tab:SWaveScatLenERT}, we can see that the scattering lengths 
...
\colrule
Kohn $(\omega = 6)$ Eq. (\ref{eq:ScatLenApprox}) \cite{VanReeth2003} & $0.0001$ & 4.334 & \,\,--- & 2.143 & \,\,--- \\
Kohn $(\omega = 6)$ Eq. (\ref{eq:EffectiveRangeShort}) \cite{VanReeth2003} & --- & 4.30 & 2.27 & 2.147 & \,\,--- \\
Kohn extrapolated \cite{VanReeth2003} & up to $0.5$ & 4.311 & 2.27 & 2.126 & 1.39 \\
...
\section{Conclusion}
We have extended the earlier Kohn variational calculations \cite{VanReeth2003,VanReeth2004}
and have presented complex Kohn variational results for Ps(1s) scattering from H(1s)
below the Ps(n=2) threshold.
%We have extended the earlier Kohn variational 
