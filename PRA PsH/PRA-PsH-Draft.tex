...
In order to keep their \todo{?} nomenclature simple, we relate the wavefunction for S-wave scattering to the Ps-H coordinates as described in figure \ref{fig:PsHCoords}. The variable $\rho$ describes the position of the center of mass of the positronium atom, i.e. $\vec{\rho} = \frac{1}{2}\left(\vec{r_1} + \vec{r_2}\right)$. A typical real trial wavefunction for S-wave scattering of positronium from hydrogen can be written as \todoi{Almost sounds like we're not using this?}
\begin{equation}
\Psi_t^\pm = \bar{S} + \tan \delta_t \, \bar{C} + \sum_{i=1}^N c_i \bar{\phi}_i^t
...
The variable $\omega$ is an integer $\geq 0$ that determines the maximum number of terms in the basis set. The short-range terms, given by $\bar{\phi}_i^t$, are a Hylleraas-type basis set\todo{Provide reference?}. For a chosen value of $\omega$, the integer powers of $r_i$ and $r_{ij}$ are constructed in such a way that
\beq
...
\todoi{Do we need to explicitly write out the Hamiltonian? We could just refer to the J. Phys. B paper (Van Reeth/Humberston 1999). They have this applied to e$^+$-He though.}
...
\todoi{Not all terms here have an $\ell$ subscript. We have to be careful that it isn't confused with the superscript $\ell$ in equation \ref{eq:PartialWavePhiBar}.}
The wavefunctions for the higher partial waves are similar in form to that of the S-wave (\ref{eq:SWaveTrialSimple}). For clarity, we remove the t subscript representing the fact that we are dealing with the trial wavefunction and assume that this is implied.

\begin{equation}
\Psi_l^\pm = \bar{S_l} + \tan \delta_l^\pm \, \bar{C_l} + \sum_{i=1}^N c_i \bar{\phi}_{i1} + \sum_{j=1}^N d_j \bar{\phi}_{j2}
\label{eq:PWaveSimple}
\end{equation}

\noindent The long-range terms are given by 
\begin{subequations}
\label{eq:PartialWaveSandCBar}
\begin{align}
\bar{S} &= \frac{1\pm P_{23}}{\sqrt{2}}Y_{\ell 0}(\theta_\rho,\phi_\rho)\Phi_{Ps}\left(r_{12}\right) \Phi_H\left(r_3\right) \sqrt{2\kappa} \,j_\ell\left(\kappa\rho\right) \label{eq:PWaveSBar} \\
\bar{C} &= \frac{1\pm P_{23}}{\sqrt{2}}Y_{\ell 0}(\theta_\rho,\phi_\rho)\Phi_{Ps}\left(r_{12}\right) \Phi_H\left(r_3\right) \sqrt{2\kappa} \,n_\ell\left(\kappa\rho\right) f_{sh}(\rho) \label{eq:PWaveCBar}.
\end{align}
\end{subequations}

\noindent The shielding factor, $f_{sh}(\rho)$, ensures that the singularity of the Neumann function is removed at the origin and is
\beq
f_{sh}(\rho) = \left[1 - \ee^{-\mu \rho} \left(1+\frac{\mu}{2}\rho\right)\right]^{m_\ell}.
...
\begin{align}
\bar{\phi}_{i1} &= \left(1 \pm P_{23}\right) Y_{\ell 0}(\theta_1) r^\ell_1 \phi_i \label{eq:PWavePhi1i}\\
\bar{\phi}_{j2} &= \left(1 \pm P_{23}\right) Y_{\ell 0}(\theta_2) r^\ell_2 \phi_j \label{eq:PartialWavePhi2j},
\end{align}
\end{subequations}
where $\phi_i$ and $\phi_j$ are given by equation \ref{eq:PhiDef}. This represent the angular momentum as being placed on either the positron ($r_1$) or on the electron on the Ps atom ($r_2$). For partial waves with $L > 1$, the angular momentum could be shared between both particles in the Ps atom \cite{??} (Schwartz). An investigation of the contribution to the final results from each symmetry \todoi{find paper Pos-He 1999 or thesis} has revealed that the mixed symmetries were not important and because of the complexity of the analytical evaluation of the various matrix elements, we have omitted the mixed symmetry term. \todoi{Why is this sentence here?} The formulation for the Complex Kohn method is the same as that expressed above.
...
$\bar{S}$, $\bar{C}$ and $\bar{\phi_i}$ are the same as in (\ref{SCphiBarDef}).  Note that this is different than Cooper et al. \cite{Cooper2010}, in that we do not use a variable $\tau$, and our $\bar{W}$ is their $\bar{T}$ but with $\bar{C}$ and $\bar{S}$ swapped, which gives a wavefunction of the correct form \todo{odd wording} (an outgoing wave).
...
\begin{figure}
	\centering
	\includegraphics[width=7in]{OtherSWaveSingletResults}
...
\begin{figure}
	\centering
	\includegraphics[width=7in]{OtherSWaveTripletResults}
...
$\kappa \cot \delta_0^\pm  = -\frac{1}{a_0^\pm} + \frac{1}{2}r_0^\pm \kappa^2 + O(\kappa^4)$
