...
\end{equation}
The scattering wavefunctions contain both the long-range terms \hl{$\widetilde{S}_\ell$}
and \hl{$\widetilde{C}_\ell$} and the short-range correlation terms $\bar{\phi}_{i\ell k}$
...
different forms depending on each: \sout{. The $\textbf{u}$ matrices and
...
differs from Lucchese \cite{Lucchese1989}.
...
terms. The final numerical results used in \hl{Ref.} \cite{VanReeth1997} are
...
Born approximation (B1) to $\tan\delta_\ell$ \hl{of} \cite{Bransden2003}
\begin{equation}
\label{eq:Born}
...
\end{equation}
We use the van der Waals coefficient \hl{of $C_6 = 34.78473$ a.u.,} as given
...
\hl{$^1S$ and $^1P$} to the resonance formula
\begin{align}
...
Our calculation yields a better value for \hl{the binding energy} than the earlier 
...
and $0.4$, the complex Kohn $^3D$ phase shifts \hl{change from positive 
to negative}.
...
Kohn phase shift \hl{curves is similar to the CC}. However, the percentage
...
We note that the percentage \hl{differences with the CC results for $^3P$ are}
...
The modified Born phase shifts are \hl{nearly} equivalent to the Born 
\hl{phase shifts}, so we show only the Born results in this figure.
...
\hl{Assuming that the initial spin state of the H(1s) target is unpolarized and 
...
In Fig.~\ref{fig:combined-cross-sections}, \hl{we show the complex Kohn
spin-weighted singlet, spin-weighted triplet and the spin-weighted integrated 
cross sections for elastic scattering, and we compare the complex Kohn
spin-weighted cross section with the corresponding CC data.}
...
are relatively well converged. Fig.~\ref{fig:diff-cross-section-2D-kappa} \hl{shows
...
cross section. Backward scattering is suppressed, and there is a very rapid
rise in the forward direction, reaching a maximum 
around 1~eV ($\kappa = 0.38)$ and staying nearly constant thereafter until
the resonance region.  We can see that most scattering occurs between 0 and 1
...
\ref{fig:combined-diff-cross-sections}, \hl{for which we find that the main 
...
\begin{figure}[H]
	...
	\caption{(Color online) The elastic differential cross section for Ps-H
scattering at selected angles (radians)}
...
Equations for the momentum transfer cross section expression are given
...
the second arctan term, \hl{since we consider only
...
$^1P$, and $^1D$ partial waves and see \hl{a similar result} to this previous work 
...
\cite{VanReeth2003}. In that paper, they noted that the singlet result \hl{lies} on a
...
ERT \hl{vdW} - Eq. (\ref{eq:EffectiveRangeLongAu}) & $0.001 - 0.009$ & $4.331 \pm 0.012$ & 2.221 & $2.137 \pm 0.008$ & 2.137 \\
...
We \hl{have determined} highly accurate $^{1,3}S$ and $^{1,3}P$ phase shifts.
...
\hl{elastic integrated or momentum transfer cross sections.}
