...
including a transition from slightly backward peaked to
...
difficulty of an H beam, although the binding energy of positronium
...
enables us to calculate the elastic differential, elastic integrated, momentum
...
14Ps14H, \hl{was used} for the singlet only and included 14 eigen- and pseudo-states of 
...
The Kohn variational method gives rigorous \hl{upper}
bounds on the scattering lengths and, except for Schwartz singularities, 
gives empirical \hl{lower} bounds on the elastic phase shifts. This means
that the wave 
...
The first Born approximation (B1) to $\tan\delta_\ell$ \cite{Bransden2003}
is given by
\begin{equation}
\label{eq:Born}
...
\end{align}
\end{widetext}
We solve these integrals \hl{in} two different ways.
...
$D$-wave, as the integrals become too singular for $\ell > 2$. The second is
a more general method from Yan and Drake \cite{Yan1997}, which works for all
partial waves.
...
shifts with respect to number of short-range terms}
...
The singlet for each of the partial waves contributes $1/4$ to the integrated,
differential, and momentum transfer cross sections, while the triplet
contributes $3/4$. We include partial waves with $\ell \leq 5$ for each of the
cross sections. 
...
sections and singlet elastic integrated \hl{cross} section.}
...
\hl{Figs.}~\ref{fig:diff-cross-section-2D-theta},
\ref{fig:combined-diff-cross-sections}, and 
\ref{fig:combined-diff-cross-sections-2}, for which we find that the main 
...
elastic cross section, $\sigma_{el}$, is given by the black curve. The complex Kohn ortho-para
conversion cross section, $\sigma_c$, is given by the dark blue (dark gray) curve. The complex
Kohn momentum transfer cross section, $\sigma_m$, is given by the light blue (light gray) curve.}
CC $\sigma_c$ data (dark blue / dark gray dash-dot) is from
Ref.~\cite{Blackwood2002}. Static exchange $\sigma_m$ data (light blue / light gray dash-dot) 
...
same as the complex Kohn curve, but the resonances are clearly shifted.
...
the second arctan term, \hl{as there is only a single resonance each}. In 
...
\cite{VanReeth2003}. In that paper, they noted that the singlet forms a
...
term to the Gauss-Laguerre quadratures. With these improved numerics, we are
able to obtain better converged phase shifts.
We also use the $S$ matrix complex Kohn variational method, which also allows
...
elastic integrated, momentum transfer, or ortho-para conversion cross
sections.
...
$^1F$ partial waves, which compare favorably to the complex 
...
values, we obtain a $^3S$ effective range noticeably larger.
