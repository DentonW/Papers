...
\author{S. J. Ward}
\email[Corresponding author: ]{sward@unt.edu}
...
complex Kohn variational method, in this section we present
a general wavefunction that can be used in the Kohn
...
nonlinear parameter $\mu$ and the power $m_\ell$
...
as being placed on either the positron ($\bm{r}_1$) or on the electron in the
Ps atom ($\bm{r}_2$, and $\bm{r}_3$
...
%$\ell + 1$ sets of short-range terms \cite{	}. For the
...
%Therefore, due to the complexity of including the mixed symmetry terms for the four-body
%system of Ps-H scattering, we do not include them for the D-wave, or in fact
%for any partial wave $\ell \ge 2$.
...
We also use an expression from Ref.~\cite{Blackwood2002} to get an estimate
...
%(\ref{eq:TrialWaveHigher}) are replaced by $N^\prime(\omega)$ to indicate
%that this is a subset of the full set of short-range terms in
%Eq. (\ref{eq:OmegaDef}).
...
\begin{table}[H]
  ...
  \begin{ruledtabular}
	...
	\end{tabular}
  \end{ruledtabular}
  \caption{Nonlinear parameters and number of terms of each symmetry in
the wavefunction for each partial wave. Numbers marked with a star indicate the
...
In retrospect, this is not surprising in that with the BO approximation for 
elastic Ps(1s)-H(1s) scattering, the direct amplitude is identically zero
\cite{Massey1954,McAlinden1996}, there are concerns with the treatment
of the exchange amplitude \cite{Oppenheimer1928,Geltman1969,Bransden1970},
and the long-range van der Waals interaction is not taken into account.  
...
\begin{figure}[H]
	\centering
	\includegraphics[width=3.4in]{singlet-cross-sections}
...
\begin{figure}[H]%
    \centering
    \subfloat[ ]{{\includegraphics[width=3.4in]{combined-diff-cross-sections-2} }}%
    %\qquad
    \subfloat[ ]{{\includegraphics[width=3.4in]{combined-diff-cross-sections} }}%
