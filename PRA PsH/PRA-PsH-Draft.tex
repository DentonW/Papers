...
We use the variants of the Kohn variational method in obtaining the estimated
errors associated with the phase shifts, resonance parameters, and
...
small interparticle distances. Fig.~\ref{fig:PsHCoords} gives the
...
\end{equation}
where we give in Table \ref{tab:Nonlinear} the value of the nonlinear
...
gives the inverse Kohn. Note that the generalized \emph{T} matrix as given
in Eq.~(\ref{eq:GenTKohn}) is different than given by
Cooper et al.\ \cite{Cooper2010}, in that our $\bar{S}$ and
$\bar{C}$ are swapped, which gives an outgoing wave.
The \textbf{u} matrix for the generalized Kohn is identical to that given by
Cooper et al.\ \cite{Cooper2010}. The \textbf{u} matrices
in Eq.~(\ref{eq:KohnU}) are similar to the
...
Recent investigation \cite{VanReeth2015} has shown 
that these mixed symmetry terms can be important for that 
system. At very low energies for e$^+$-H, the mixed symmetry terms are
negligible, contributing less than 1\% to the phase shifts at $\kappa = 0.1$,
and near the Ps formation threshold, contribute about 10\% of the final value
of the phase shifts. The $^1D$ phase shifts are more sensitive to inclusion
of the mixed symmetry terms than the $^3D$ phase shifts. Likewise,
investigation into e$^-$-H scattering \cite{VanReeth2015} reveals a similar
pattern. 

In their work on e$^+$-He scattering, treated similar to the four-body system
...
contributions of the mixed symmetry terms in the two other systems appear to be 
opposite of the Ps-H system in that the $^3D$ Ps-H phase shifts are
significantly lower than the results of Ref.~\cite{Walters2004}
(see Sec.~\ref{sec:PhaseCross}).
...
integrated elastic cross sections away from the D-wave resonance.
...
\end{equation}
It can be shown that the variation $I$, $\delta I = I[\Psi^t] - I[\Psi] = I[\Psi^t]$,
is given by
\begin{equation}
