...
\begin{figure}[H]
	...
	\caption{(Color online) Comparison of cross sections calculated using complex Kohn}
	\label{fig:cross-section-comparisons}
\end{figure}

As pointed out in Ref. \cite{Buckman1989}, $\sigma_m = \sigma_{el}$ for zero energy. From Fig. \ref{fig:cross-section-comparisons}, we can see that this is the case. For very low energies ($E < 10^{-6}$ eV), $\sigma_m = \sigma_{el} = 32.45$. The ortho-para conversion cross section, $\sigma_c$, is much lower than the elastic and momentum transfer cross sections. Both the elastic and ortho-para conversion cross sections can be compared to that of Refs. \cite{Hara1975} and \cite{Fraser1961}. To the best of our knowledge, the momentum transfer cross sections have not been computed for this system before.
...
Complex rotation (Yan and Ho 1998) \cite{Yan1999} $^\dagger$ & $4.2850 \pm 0.0014$ & $0.0435 \pm 0.0027$ & $5.0540 \pm 0.0027$ & $0.0925 \pm 0.0054$ \\
...
Similar to the work by Van Reeth and Humberston on $^{1,3}$S \cite{VanReeth2003}, we use Eq. (\ref{eq:ScatLen}) to find the $^{1,3}$S and $^{1,3}$P scattering lengths for small values of $\kappa$, given in Table \ref{tab:SWaveScatLenERT}. For the P-wave, we were unable to use the very low $\kappa = 0.0001$ that we use for the S-wave due to numerical inaccuracies. We see that to the accuracy reported, the $^{1,3}$S scattering lengths have converged well with respect to $\kappa$.
