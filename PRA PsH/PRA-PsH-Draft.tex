...
Eq.~\ref{eq:TrialWaveHigher} with a set of spherical harmonics and
Clebsch-Gordan coefficients given by
\begin{align}
\label{sec:MixedTerms}
\psi(1,1,2,0) = Y&_{1,-1}(\theta_1,\varphi_1) Y_{1,+1}(\theta_2,\varphi_2)
...
&+ Y_{1,+1}(\theta_1,\varphi_1) Y_{1,-1}(\theta_2,\varphi_2)
   \left< 1,+1,1,-1 | 2,0 \right>.
\end{align}
We did not explicitly include the mixed terms for Ps-H scattering due 
...
the inclusion of the mixed terms may improve the convergence of the $^{1,3}D$ 
phase shifts. Likewise, investigation into e$^-$-H scattering
\cite{VanReeth2015} indicates that the mixed terms are negligible at very low
...
1\% to the $^3D$ phase shifts for e$^-$-H scattering. The evaluation of the 
various matrix elements are more complicated for Ps-H scattering than these 
other systems, particularly evaluating the mixed symmetry terms with the 
...
fit the phase shifts are to Eq.~(\ref{eq:GaoZEqn}) to determine $K_\ell^0(E)$ 
for each $\kappa$ value. Since it varies slowly with energy, $K_\ell^0(E)$ is 
...
K_\ell^0(E_{\bm \kappa}) = K_\ell^0(0) + {K_\ell^0}^\prime(0) E_{\bm \kappa}
  + \ldots.
\end{equation}
We retain the first two terms in the expansion only. From this, $K_\ell^0(0)$
and ${K_\ell^0}^\prime(0)$ are determined. In Ref. \cite{Gao1998a},
...
That said, the mixed terms should be added for partial waves $\ell \geq 2$,
even if they make little difference to the integrated elastic cross section.
...
explicitly including mixed terms into the trial wavefunction. One would 
expect that the mixed terms will improve the convergence of the $^{1,3}D$ 
